...
| `@sakeetech/viva-payments-core/payments` | [`Payments`](#payments) — mode-aware Smart Checkout client (`createOrder`, `retrieveTransaction`, `retrieveOrder`, `refundPayment`, `cancelOrder`). |
...
| `retrieveOrder(orderCode, opts?)` | `GET /api/orders/{orderCode}` on the **legacy host** with Basic auth (no v2/OAuth2 equivalent — that route 404s). Requires `legacyClient`. Amounts are major-unit floats with no currency in the body; pass `opts.currencyCode` for non-2-decimal currencies. Idempotent. |
