...
export class Payments {
  ...
  async createOrder(
    ...
    if (req.customerEmail !== undefined) wireBody['email'] = req.customerEmail;
    if (req.customerPhone !== undefined) wireBody['phone'] = req.customerPhone;
    if (req.customerFullName !== undefined) wireBody['fullName'] = req.customerFullName;
    if (req.successUrl !== undefined) wireBody['successUrl'] = req.successUrl;
    if (req.failureUrl !== undefined) wireBody['failureUrl'] = req.failureUrl;
    ...
    if (req.preselectedPaymentMethod !== undefined) wireBody['preselectedPaymentMethod'] = req.preselectedPaymentMethod;
    ...
    const raw = await this.client.request<{ OrderCode: OrderCode }>({
      method: 'POST',
      path,
      query,
      body: wireBody,
  ...
  async retrieveTransaction(
    ...
    return {
      transactionId: (raw['transactionId'] ?? raw['TransactionId']) as TransactionId,
      orderCode,
  ...
  async refundPayment(
    ...
    if (!this.legacyClient) {
      throw new VivaValidationError({
        message:
          'refundPayment requires a legacyClient (Basic auth with MerchantId + ApiKey). ' +
    ...
    const formBody: Record<string, string | number | bigint | undefined> = {
    ...
    const result = await this.legacyClient.request<LegacyRefundRaw>({
      method: 'POST',
