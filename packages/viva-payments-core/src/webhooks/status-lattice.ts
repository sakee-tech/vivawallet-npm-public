...
// ---------------------------------------------------------------------------
// Status letter → plugin status mapping
// ---------------------------------------------------------------------------

type MappedStatus =
  | { status: Exclude<VivaTransactionStatus, 'disputed'>; claimSubstate: null }
  | { status: 'disputed'; claimSubstate: VivaClaimSubstate };
 ...
 * @see references/viva-docs/md/wh-transaction-payment-created.txt:398
 */
export function mapStatusLetter(letter: VivaStatusLetter): {
 ...
 */
const TERMINAL_STATES: ReadonlySet<VivaTransactionStatus> = new Set<VivaTransactionStatus>([
  'failed',
  'cancelled',
  'disputed',
]);
 ...
 * @see references/viva-docs/md/wh-transaction-payment-created.txt:398
 */
const LATTICE: ReadonlyMap<VivaTransactionStatus, ReadonlySet<VivaTransactionStatus>> = new Map<
  VivaTransactionStatus,
  ReadonlySet<VivaTransactionStatus>
>([
  ['initiated', new Set<VivaTransactionStatus>(['authorized', 'captured', 'failed'])],
  ['authorized', new Set<VivaTransactionStatus>(['captured', 'cancelled', 'failed', 'disputed'])],
  ['captured', new Set<VivaTransactionStatus>(['refunded', 'disputed'])],
  ['refunded', new Set<VivaTransactionStatus>(['disputed'])],
  // Terminal states — empty sets (no forward transitions).
  ['failed', new Set<VivaTransactionStatus>()],
  ['cancelled', new Set<VivaTransactionStatus>()],
  ['disputed', new Set<VivaTransactionStatus>()],
]);
...
const _ALL_STATUSES: readonly VivaTransactionStatus[] = [
  'initiated',
  'authorized',
  'captured',
  'refunded',
 ...
 */
function computeAncestors(target: VivaTransactionStatus): Set<VivaTransactionStatus> {
  const ancestors = new Set<VivaTransactionStatus>();
  const queue: VivaTransactionStatus[] = [];

  // Find direct parents of `target`.
  for (const [from, allowed] of LATTICE) {
    if (allowed.has(target)) {
      queue.push(from as VivaTransactionStatus);
    }
  }
  ...
  while (queue.length > 0) {
    ...
    // Walk further up.
    for (const [from, allowed] of LATTICE) {
      if (allowed.has(node) && !ancestors.has(from as VivaTransactionStatus)) {
        queue.push(from as VivaTransactionStatus);
      }
    }
  }
...
const _ANCESTORS: ReadonlyMap<VivaTransactionStatus, ReadonlySet<VivaTransactionStatus>> = new Map(
  _ALL_STATUSES.map((s) => [s, computeAncestors(s)]),
);
...
export type StatusTransitionResult =
  | { ok: true; next: VivaTransactionStatus }
  | {
      ok: false;
...
export function applyStatusTransition(
  current: VivaTransactionStatus,
  incomingLetter: VivaStatusLetter,
): StatusTransitionResult & { claimSubstate: VivaClaimSubstate | null } {
