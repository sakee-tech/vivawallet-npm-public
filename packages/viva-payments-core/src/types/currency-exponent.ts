 ...
 */
export function minorUnitExponent(currencyCode: string | number | undefined): number {
  if (currencyCode === undefined) return 2;
...
export function majorToMinor(
  ...
  if (!Number.isFinite(n)) return 0n as MinorUnits;
  return BigInt(Math.round(n * factor)) as MinorUnits;
}
