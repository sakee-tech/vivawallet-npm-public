...
export interface CreateOrderRequest {
  ...
  /**
   * Seconds the payment order stays open before it expires. Emitted on the wire
   * as `paymentTimeout` (payment-isv-api.yaml:5416).
   */
...
export interface RetrieveTransactionResponse {
   ...
   *
   * NOTE on the wire: Viva returns `Amount` in MAJOR units as a decimal
   * (e.g. 23.17), unlike the createOrder REQUEST which takes minor units. The
   * client converts major→minor exponent-aware on parse (see
   * `majorToMinor`), so this field is always minor units to consumers.
...
/**
 * Response from Retrieve Order — `GET /api/orders/{orderCode}` on the LEGACY
 * host (Basic auth: Merchant Basic, or Reseller 3-part Basic for ISV).
 *
 * There is NO OAuth2/v2 equivalent — `GET /checkout/v2/orders/{oc}` 404s, so the
 * order-by-code resource lives only on the legacy host (same host/auth as
 * cancelOrder).
 *
 * Amount note: Viva returns `RequestAmount`/`TipAmount` as MAJOR-unit floats and
 * the response carries NO currency code, so they are converted to minor units
 * using the caller-supplied currency exponent (default: 2 decimals). Pass
 * `currencyCode` to `retrieveOrder` for zero-decimal (JPY) or three-decimal
 * (KWD) currencies.
 *
 * @see docs/internal/payment-isv-api.yaml:786 (GET /api/orders/{order_code})
 */
export interface RetrieveOrderResponse {
  /** Order code (int64) — echoed from the request path (precision-safe). */
  readonly orderCode: OrderCode;
  /**
   * Net payment amount of the order in integer minor units.
   * Wire value is a major-unit float; converted via the currency exponent.
   */
  readonly requestAmount: MinorUnits;
  /** Tip amount in integer minor units (0n when not applicable). */
  readonly tipAmount: MinorUnits;
  /**
   * Order state:
   *   0 = Pending, 1 = Expired, 2 = Canceled, 3 = Paid.
   */
  readonly stateId: number;
  /** Payment source code (e.g. "Default"). */
  readonly sourceCode?: string;
  /** Reporting tags attached to the order. */
  readonly tags?: readonly string[];
  /** ISO 3166-1 alpha-2 request language (e.g. "en-GB"). */
  readonly requestLang?: string;
  /** Merchant-facing reference string. */
  readonly merchantTrns?: string;
  /** Customer-facing description. */
  readonly customerTrns?: string;
  /** Number of installments offered (if applicable). */
