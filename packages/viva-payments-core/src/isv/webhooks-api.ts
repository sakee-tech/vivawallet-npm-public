...
export class IsvWebhooks {
  constructor(private readonly client: IsvHttpClient) {}
   ...
   * is exceeded.
   */
  async registerWebhook(req: RegisterWebhookRequest): Promise<void> {
   ...
   * registered URL.
   */
  async getVerificationKey(): Promise<RetrieveWebhookKeyResponse> {
    return this.client.request<RetrieveWebhookKeyResponse>({
      method: 'GET',
