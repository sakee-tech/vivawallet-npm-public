...
import {
  VivaApiError,
  VivaValidationError as _VivaValidationError,
  VivaRateLimitError,
...
export class IsvHttpClient {
   ...
   */
  private _vivaLastCorrelationId: string | undefined = undefined;

  /**
   * Last observed `x-viva-eventid` header value.
   * Updated after every successful HTTP response. Used by `requestWithMeta`.
   */
  private _vivaLastEventId: string | undefined = undefined;
  ...
  constructor(config: IsvHttpClientConfig) {
    ...
    if (config.dispatcher) {
      ...
      this.dispatcher = getApiDispatcher(config.environment) as Dispatcher;
      this.fetchImpl = config.fetchImpl ?? (undiciFetch as unknown as typeof fetch);
    }
  }
   ...
   * @see references/viva-docs/md/isv-partner-program.txt:104
   */
  async requestWithMeta<T>(opts: RequestOptions): Promise<{ data: T; vivaCorrelationId: string | undefined; vivaEventId: string | undefined }> {
    const endpoint = opts.endpoint ?? `${opts.method} ${opts.path}`;
    const data = await this.metrics.timeAsync(
      'viva_api_request_duration_seconds',
      () => this._withAuthRefresh<T>(opts),
      { endpoint },
    );
   ...
   * @see references/viva-docs/md/isv-credentials.txt:107
   */
  private async _withAuthRefresh<T>(opts: RequestOptions): Promise<T> {
    const token = await this.authStrategy.getBearerToken();
    try {
      return await this._executeWithRetry<T>(opts, token);
    } catch (err) {
      if (err instanceof VivaAuthError && (err as VivaAuthError).httpStatus === 401) {
        // Force-refresh and retry once.
        const freshToken = await this.authStrategy.getBearerToken({ forceRefresh: true });
        return await this._executeWithRetry<T>(opts, freshToken);
      }
      throw err;
    }
  }
   ...
   * @see references/viva-docs/md/isv-partner-program.txt:104 (Auth Flow line 319)
   */
  private async _executeWithRetry<T>(opts: RequestOptions, token: string): Promise<T> {
    let attempt = 0;
  ...
  private _parseErrorBody(text: string): { vivaCode?: string | undefined; vivaMessage?: string | undefined } {
    if (!text) return {};
    try {
   ...
   */
  private _isBeforeAck(_err: unknown): boolean {
    return true;
  }
