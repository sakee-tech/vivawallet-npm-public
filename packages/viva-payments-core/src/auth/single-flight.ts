...
// ---------------------------------------------------------------------------
// Redis lock interface (injectable, no concrete Redis dep in this package)
// ---------------------------------------------------------------------------
 ...
 */
export const noopRedisLock: RedisLockClient = {
  acquire() {
...
export class AsyncMutex {
  ...
  acquire(): Promise<() => void> {
    ...
    const ourTurn = new Promise<void>((res) => {
      resolve = res;
    });
...
type Locks = { local: AsyncMutex } | { redis: RedisLockClient };
 ...
 */
export function singleFlight<T>(key: string, fn: () => Promise<T>, locks: Locks): Promise<T> {
  if ('local' in locks) {
    // ---- In-process path ----
    ...
    return (async () => {
      ...
      if (lockResult !== null) {
        ...
        const cached = await locks.redis.pollCacheUntil<T>(
          async () => null, // caller provides a real getter at the strategy level
          100,
          10_000,
        );
