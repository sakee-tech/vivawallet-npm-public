 ...
 *       and response parsing into FastRefundResponse (only `transactionId` per spec).
...
describe('FastRefundClient', () => {
  it('200 happy path: POSTs to fastrefund path with correct body and returns parsed response', async () => {
    ...
    // eventId and amount are NOT in the real Viva response (spec fastrefund_success
    // at payment-api.yaml:13640 has only transactionId). No assertions for them.

    // idempotencyKey IS a body field per spec (payment-api.yaml:9305) — and also a header.
  ...
  it('URL contains URL-encoded transactionId path segment', async () => {
    ...
    pool
      ...
      .reply(
        ...
        (opts) => {
          capturedPath = opts.path;
          return JSON.stringify({ transactionId: 'refund-tx' });
        },
        { headers: { 'Content-Type': 'application/json' } },
      );
