...
describe('IsvPayments', () => {
    ...
    pool
      ...
      .reply(
        ...
        JSON.stringify({
          ...
          // lowercase orderCode matches probe-captured fixture retrieve-transaction-finished.json
          // (spec payment-isv-api.yaml:1160 confirms camelCase for v2 ISV endpoint)
  ...
  it('retrieveTransaction derives cardType string from CardTypeId', async () => {
    ...
    pool
      ...
      .reply(
        ...
        JSON.stringify({
          transactionId: TEST_TRANSACTION_ID,
          // lowercase orderCode per probe-captured fixture (v2 ISV endpoint uses camelCase)
   ...
   */
  it('refundPayment (partial) → DELETEs to LEGACY HOST with amount/sourceCode in query string', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    ...
    legacyPool
      ...
      .reply(
        ...
        (opts) => {
          ...
          // Viva returns Amount in MAJOR units as a decimal (€5.00), not minor
          // units — the client converts back to minor (500n).
          return JSON.stringify({ TransactionId: 'refund-tx-uuid', StatusId: 'F', Amount: 5.0 });
        },
        { headers: { 'Content-Type': 'application/json' } },
      );
    ...
    // Assert query string carries amount + sourceCode (Viva Cancel transaction).
    const query = new URL(`http://x${capturedPath}`).searchParams;
    expect(query.get('amount')).toBe('500');
    expect(query.get('sourceCode')).toBe('Default');
   ...
   * @see references/viva-docs/md/tut-create-recurring-payment.txt:288
   */
  it('refundPayment (full refund) → no amount in query, sourceCode=Default', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    ...
    // amount must NOT be in the query for a full refund; sourceCode still present.
    const query = new URL(`http://x${capturedPath}`).searchParams;
   ...
   */
  it('refundPayment — custom sourceCode is included in query string', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    ...
    const query = new URL(`http://x${capturedPath}`).searchParams;
    expect(query.get('sourceCode')).toBe('MySource');

    await agent.close();
  });

  /**
   * DEV-2: ISV refund authenticates with the Reseller Basic credential
   * `base64(ResellerId:ConnectedMerchantId:ResellerApiKey)` when a reseller-variant
   * client is injected via opts.legacyClient.
   * @see docs/internal/payment-isv-api.yaml:2650
   */
  it('refundPayment — reseller-variant legacyClient sends 3-part reseller Basic auth', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);

    const RESELLER_ID = 'reseller-uuid-aaaa';
    const CONNECTED_MERCHANT_ID = 'connected-merchant-bbbb';
    const RESELLER_API_KEY = 'reseller-secret-key';
    ...
    legacyPool
      ...
      .reply(
        ...
        (opts) => {
          const headers = opts.headers as Record<string, string> | undefined;
          capturedAuthHeader = headers?.['authorization'] ?? headers?.['Authorization'] ?? '';
          return JSON.stringify({ TransactionId: 'refund-tx-reseller', StatusId: 'F' });
        },
        { headers: { 'Content-Type': 'application/json' } },
      );
    ...
    const resellerClient = new LegacyBasicClient({
    ...
    const payments = buildPaymentsWithLegacy(agent);
    const result = await payments.refundPayment(TEST_TRANSACTION_ID, {
      merchantId: CONNECTED_MERCHANT_ID as MerchantId,
      amountMinor: 300n,
      idempotencyKey: 'key-reseller',
      legacyClient: resellerClient,
    });

    expect(result.transactionId).toBe('refund-tx-reseller');
    const expected =
      'Basic ' +
      Buffer.from(`${RESELLER_ID}:${CONNECTED_MERCHANT_ID}:${RESELLER_API_KEY}`).toString('base64');
  ...
  /**
   * Test 5: cancelOrder requires a legacyClient (Basic auth) — with none wired it
   * throws VivaValidationError before any HTTP call.
   *
   * cancelOrder routes to the LEGACY HOST (DELETE /api/orders/{oc}) with Basic
   * auth — the v2/OAuth2 route DELETE /checkout/v2/orders/{oc} does NOT exist
   * (verified 404 against live demo). The real legacy-host contract (merchant +
   * reseller Basic, idempotency) is exercised by the live cancel test against the
   * demo sandbox — see test/live/card/retrieve-refund-cancel.live.test.ts.
   *
   * @see docs/internal/viva-cancel-probes/viva-cancel-order-findings.md          (v2/OAuth2 route → 404)
   * @see docs/internal/viva-cancel-probes/viva-cancel-isv-reseller-findings.md   (legacy reseller → 200, idempotent)
   * @see references/viva-docs/md/webhooks-for-payments.txt:205 (4865 event)
   */
  it('cancelOrder without a legacyClient throws VivaValidationError (no HTTP call)', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const payments = buildPayments(agent); // OAuth2-only, no legacyClient wired
    await expect(payments.cancelOrder(12345n)).rejects.toBeInstanceOf(VivaValidationError);
    await agent.close();
  });

  // -------------------------------------------------------------------------
  // Response amount is MAJOR units (decimal) — convert exponent-aware to minor.
  // Regression for sakee-tech/vivawallet-npm-public#20 + the same class in
  // refundPayment, plus zero-decimal currency handling.
  // -------------------------------------------------------------------------

  it('retrieveTransaction converts a decimal major-unit amount to minor units (no BigInt throw) — #20', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const pool = agent.get(API_HOST);
    pool
      .intercept({ path: `/checkout/v2/isv/transactions/${TEST_TRANSACTION_ID}?merchantId=${TEST_MERCHANT_ID}`, method: 'GET' })
      .reply(
        200,
        JSON.stringify({ OrderCode: 7196730053856611, statusId: 'F', amount: 23.17, currencyCode: '826', transactionTypeId: 5 }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPayments(agent);
    const tx = await payments.retrieveTransaction(TEST_TRANSACTION_ID, { merchantId: TEST_MERCHANT_ID });
    expect(tx.amount).toBe(2317n); // £23.17 → 2317 minor (GBP, 2 decimals)
    expect(typeof tx.amount).toBe('bigint');
    await agent.close();
  });

  it('retrieveTransaction honours zero-decimal currencies (JPY = ×1, not ×100)', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const pool = agent.get(API_HOST);
    pool
      ...
      .reply(
        200,
        JSON.stringify({ OrderCode: 5555, statusId: 'F', amount: 5000, currencyCode: 392, transactionTypeId: 5 }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPayments(agent);
    const tx = await payments.retrieveTransaction(TEST_TRANSACTION_ID, { merchantId: TEST_MERCHANT_ID });
    expect(tx.amount).toBe(5000n); // ¥5000 major == 5000 minor (JPY, 0 decimals)
    expect(tx.currencyCode).toBe('392'); // numeric code coerced to string
    await agent.close();
  });

  it('retrieveTransaction falls back to opts.merchantId when the response omits it', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const pool = agent.get(API_HOST);
    pool
      ...
      .reply(
        200,
        JSON.stringify({ OrderCode: 5555, statusId: 'F', amount: 1.0, currencyCode: '978', transactionTypeId: 5 }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPayments(agent);
    const tx = await payments.retrieveTransaction(TEST_TRANSACTION_ID, { merchantId: TEST_MERCHANT_ID });
    expect(tx.merchantId).toBe(TEST_MERCHANT_ID);
    await agent.close();
  });

  it('refundPayment converts a decimal major-unit Amount to minor units (no throw) — #20 class', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    legacyPool
      .intercept({ path: new RegExp(`^/api/transactions/${TEST_TRANSACTION_ID}`), method: 'DELETE' })
      .reply(
        200,
        JSON.stringify({ TransactionId: 'r', StatusId: 'F', Amount: 23.17, CurrencyCode: 826, ErrorCode: 0, Success: true }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPaymentsWithLegacy(agent);
    const result = await payments.refundPayment(TEST_TRANSACTION_ID, { merchantId: TEST_MERCHANT_ID, amountMinor: 2317n, idempotencyKey: 'k' });
    expect(result.amount).toBe(2317n);
    await agent.close();
  });

  // -------------------------------------------------------------------------
  // Success gate: a 200 carrying Success:false / ErrorCode!=0 is a FAILURE.
  // -------------------------------------------------------------------------

  it('refundPayment throws VivaApiError on a 200 body with Success:false', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    legacyPool
      ...
      .reply(
        200,
        JSON.stringify({ TransactionId: 'r', ErrorCode: 4, ErrorText: 'refund amount exceeds total', Success: false }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPaymentsWithLegacy(agent);
    await expect(
      payments.refundPayment(TEST_TRANSACTION_ID, { merchantId: TEST_MERCHANT_ID, amountMinor: 500n, idempotencyKey: 'k' }),
    ).rejects.toBeInstanceOf(VivaApiError);
    await agent.close();
  });

  it('cancelOrder throws VivaApiError on a 200 body with Success:false', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    legacyPool
      .intercept({ path: /^\/api\/orders\/12345/, method: 'DELETE' })
      .reply(
        200,
        JSON.stringify({ OrderCode: 12345, ErrorCode: 102, ErrorText: 'cannot cancel', Success: false }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPaymentsWithLegacy(agent);
    await expect(payments.cancelOrder(12345n)).rejects.toBeInstanceOf(VivaApiError);
    await agent.close();
  });

  it('cancelOrder succeeds (idempotent) on a 200 body with Success:true', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    legacyPool
      ...
      .reply(
        200,
        JSON.stringify({ OrderCode: 12345, ErrorCode: 0, ErrorText: null, Success: true }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPaymentsWithLegacy(agent);
    const res = await payments.cancelOrder(12345n);
    // Legacy host echoes OrderCode as a JSON number (the legacy client does not
    // run the bigint-safe parser); the success gate did not throw.
    expect(Number(res.orderCode)).toBe(12345);
    expect(res.errorCode).toBe(0);
    await agent.close();
  });

  // -------------------------------------------------------------------------
  // retrieveOrder — GET /api/orders/{orderCode} on the legacy host (Basic auth).
  // @see docs/internal/payment-isv-api.yaml:786
  // -------------------------------------------------------------------------

  it('retrieveOrder maps the get_order body, echoes the path orderCode, converts major→minor', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    legacyPool
      .intercept({ path: /^\/api\/orders\/3958121726095215/, method: 'GET' })
      .reply(
        200,
        JSON.stringify({
          ...
          MaxInstallments: 12,
          RequestAmount: 1.0, // £1.00 major → 100 minor (default exp 2)
    ...
    const payments = buildPaymentsWithLegacy(agent);
    const res = await payments.retrieveOrder(3958121726095215n);

    expect(res.orderCode).toBe(3958121726095215n); // echoed path param, precision-safe
    expect(res.requestAmount).toBe(100n);
    expect(res.tipAmount).toBe(0n);
    expect(res.stateId).toBe(3);
    expect(res.sourceCode).toBe('Default');
    expect(res.merchantTrns).toBe('Your merchant reference');
    expect(res.maxInstallments).toBe(12);
    await agent.close();
  });

  it('retrieveOrder honours a zero-decimal currency (JPY ×1 via opts.currencyCode)', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    legacyPool
      .intercept({ path: /^\/api\/orders\/5555/, method: 'GET' })
      .reply(
        200,
        JSON.stringify({ OrderCode: 5555, RequestAmount: 5000, TipAmount: 0, StateId: 0 }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    const payments = buildPaymentsWithLegacy(agent);
    const res = await payments.retrieveOrder(5555n, { currencyCode: 392 });
    expect(res.requestAmount).toBe(5000n); // ¥5000 major == 5000 minor, no ×100
    await agent.close();
  });

  it('retrieveOrder without a legacyClient throws VivaValidationError (no HTTP call)', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const payments = buildPayments(agent); // OAuth2-only, no legacyClient wired
    await expect(payments.retrieveOrder(12345n)).rejects.toBeInstanceOf(VivaValidationError);
    await agent.close();
  });

  it('retrieveOrder surfaces a 404 (order not found) as VivaApiError', async () => {
    const agent = new MockAgent();
    agent.disableNetConnect();
    const legacyPool = agent.get(LEGACY_HOST);
    legacyPool
      .intercept({ path: /^\/api\/orders\/9999/, method: 'GET' })
      .reply(404, JSON.stringify({ Message: 'Not found' }), { headers: { 'Content-Type': 'application/json' } });
    const payments = buildPaymentsWithLegacy(agent);
    await expect(payments.retrieveOrder(9999n)).rejects.toBeInstanceOf(VivaApiError);
    await agent.close();
  });
});
...
describe('IsvPayments — 401 reseller fallback (D15)', () => {
  // -------------------------------------------------------------------------
  // cancelOrder — no longer part of the OAuth2 401-fallback surface.
  //
  // cancelOrder now calls the legacy host (DELETE /api/orders/{oc}) with Basic
  // auth directly (the v2/OAuth2 route 404s — verified live). It never touches
  // the primary/secondary OAuth2 clients, so the 401-reseller-fallback cases that
  // used to live here no longer apply. Its contract is covered by the
  // legacy-host tests above (Test 5 / 5b / 5c) and the live cancel test.
  // -------------------------------------------------------------------------
