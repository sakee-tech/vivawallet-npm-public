...
// ---------------------------------------------------------------------------
// cancelOrder — covered by the legacy-host unit tests in test/isv/payments.test.ts
// (Test 5 family) and the live cancel test. cancelOrder no longer uses the
// v2/OAuth2 client these merchant-mode helpers build (the v2 route 404s — it now
// calls DELETE /api/orders/{oc} on the legacy host with Basic auth), so there is
// nothing mode-specific to assert through this OAuth2-only harness.
// ---------------------------------------------------------------------------
...
describe('Payments (mode=isv) — merchantId required', () => {
  ...
  /**
   * Test 9: cancelOrder without a legacyClient throws VivaValidationError before
   * any HTTP call. cancelOrder authenticates via a legacy-host Basic client (the
   * v2/OAuth2 route 404s); these helpers wire none, so the guard fires.
   */
...
describe('Payments URL contract — snapshot per mode', () => {
  ...
  it('exact URL strings per mode (merchant vs isv)', async () => {
    ...
    // merchant mode: createOrder, retrieveTransaction.
    // (cancelOrder is NOT exercised here — it now uses the legacy host, not this
    // OAuth2 harness; its URL contract lives in the legacy-host + live tests.)
    pool
      .intercept({ path: '/checkout/v2/orders', method: 'POST' })
    ...
    // isv mode: createOrder, retrieveTransaction.
    pool
      .intercept({ path: `/checkout/v2/isv/orders?merchantId=${TEST_MERCHANT_ID}`, method: 'POST' })
    ...
    // all four URLs matched their interceptors — contract holds.
