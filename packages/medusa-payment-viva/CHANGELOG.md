...
- **The plugin now builds as a real Medusa plugin so its resource layer actually
  mounts.** The package shipped a `tsc → dist/` build, but Medusa loads a
  plugin's loaders, subscribers and API routes only from `<pkg>/.medusa/server`.
  As shipped (≤0.3.0) the OAuth2 loader, the webhook subscriber, and the
  admin/internal/webhook routes never loaded in a real Medusa app — webhook-driven
  settlement could not run. The build now uses `medusa plugin:build`, the package
  exports `./.medusa/server/...` (with `./admin`), and a boot test
  (`boots-in-medusa`) loads the plugin inside a real Medusa instance and resolves
  the config off a request scope. The webhook subscriber is also exported as the
  default export Medusa requires (it was a named export and was silently skipped
  on boot).
- **No more runtime `process.env` reads (#21).** The loader, webhook subscriber,
  `_mode-gate`, and the admin/internal API routes read Viva config from
  `process.env` on every request. They now resolve a single `vivaPluginConfig`
  registered on the DI container at boot via `resolveVivaConfig(req.scope)`
  (subscribers/loaders use the container). `loadConfigFromEnv` remains only at
  the boot-time registration point and the `register-webhooks` CLI. This makes
  config come from plugin options, matching the Stripe/Mollie pattern, and keeps
  per-tenant config correct under module isolation.

## 0.3.0

### Bug Fixes

- **Source creation no longer crashes / now persists the code (#24).** The
  admin source route previously read `sourceCode` from the `createEcommerceSource`
  response — but Viva returns an empty `200` body, so that was `undefined`. The
  route now requires `sourceCode` in the request, normalizes a number or string
  to a quoted 4-digit string for the wire, and uses the caller-supplied code.
  The HTTP API stays backward-compatible (a numeric `sourceCode` is still
  accepted).
...
- Bumps `@sakeetech/viva-payments-core` to `0.2.9` (amount major→minor response
  parsing, `retrieveOrder`, refund/cancel success-body gates). No medusa-adapter
