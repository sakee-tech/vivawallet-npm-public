  ...
  for (let i = 0; i < count; i++) {
    if (opts.registerFailAtIndex === i) {
      apiPool
        ...
        .reply(
          500,
          JSON.stringify({ status: 500, message: 'Internal Server Error', eventId: 20068 }),
          { headers: { 'Content-Type': 'application/json' } },
        );
    } else {
...
describe('run() CLI integration — ISV mode (regression)', () => {
  it('1 — dry-run: 6 REGISTER in plan, exit 1', async () => {
  ...
  it('2 — apply: registers 6 → exit 0; re-run also registers 6 (no list endpoint, plugin re-posts)', async () => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});

    const { agent: agent1 } = buildMockAgent({ registerCount: 6 });
    const exitCode1 = await run(OPTS_APPLY, VALID_ENV, agent1);
    await agent1.close();
    expect(exitCode1).toBe(0);

    const { agent: agent2 } = buildMockAgent({ registerCount: 6 });
    const exitCode2 = await run(OPTS_APPLY, VALID_ENV, agent2);
    await agent2.close();
    expect(exitCode2).toBe(0);
  });

  it('3 — apply fails midway at the 4th register → exit 2', async () => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});

    const { agent } = buildMockAgent({ registerCount: 6, registerFailAtIndex: 3 });
