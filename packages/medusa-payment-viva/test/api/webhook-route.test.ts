...
function makeReqRes(opts: {
  ...
  rawBody?: Buffer;
  /** #21: Viva plugin config resolved from DI container instead of process.env. */
  config?: Partial<VivaPluginConfig>;
}): { req: MedusaRequest; res: MedusaResponse; emitted: EmittedEvent[] } {
  const emitted: EmittedEvent[] = [];

  const scope = {
    resolve: (key: string) => {
      if (key === 'vivaPluginConfig') return opts.config ?? undefined;
      if (key === 'logger') {
...
describe('GET /viva/webhook', () => {
  it('1. returns { Key } 200 when key is set', async () => {
    // #21: key comes from DI-resolved config, not process.env
    const { req, res } = makeReqRes({
      method: 'GET',
      config: { webhookVerificationKey: VERIFICATION_KEY } as VivaPluginConfig,
    });
  ...
  it('2. returns 500 when webhookVerificationKey is absent from config', async () => {
    // #21: absent config → key resolves to '' → 500
...
describe('POST /viva/webhook', () => {
  it.skipIf(!pgReachable)('3. 1796 happy: inserts row and emits event', async () => {
    process.env['DATABASE_URL'] = connString; // infra — not plugin config

    const envelope = buildEnvelope();
    const body = Buffer.from(JSON.stringify(envelope));

    // #21: environment + bypass come from DI config, not process.env
    const { req, res, emitted } = makeReqRes({
      ...
      config: {
        environment: 'demo',
        webhookIpAllowlistBypass: true,
        webhookVerificationKey: VERIFICATION_KEY,
      } as VivaPluginConfig,
    });
  ...
  it.skipIf(!pgReachable)('4. duplicate MessageId: no INSERT, no emit', async () => {
    ...
    const { req: req1, res: res1, emitted: emitted1 } = makeReqRes({ method: 'POST', ip: '127.0.0.1', rawBody: body, config: postConfig });
    await POST(req1, res1 as MedusaResponse);
    expect(emitted1.length).toBe(1);

    // Second POST — same MessageId, should be deduped
    const { req: req2, res: res2, emitted: emitted2 } = makeReqRes({ method: 'POST', ip: '127.0.0.1', rawBody: body, config: postConfig });
  ...
  it.skipIf(!pgReachable)('7. Non-allowlisted IP: 403, no INSERT', async () => {
    ...
    // #21: production env + no bypass → IP check is active
    const { req, res } = makeReqRes({
      method: 'POST',
      ...
      config: {
        ...
        webhookIpAllowlistBypass: false,
        webhookVerificationKey: VERIFICATION_KEY,
      } as VivaPluginConfig,
    });
  ...
  it.skipIf(!pgReachable)(
    ...
    async () => {
      ...
      // At depth=0 (omitted from config → default 0) the helper ignores X-F-F
      // entirely and uses the socket IP (8.8.8.8) which is NOT in the
      // production allowlist → 403.
      // #21: trustedProxyDepth=0 is the default when absent from config
      const { req, res } = makeReqRes({
        method: 'POST',
        ip: '8.8.8.8',
        ...
        config: {
          ...
          webhookIpAllowlistBypass: false,
          webhookVerificationKey: VERIFICATION_KEY,
          // trustedProxyDepth absent → defaults to 0
        } as VivaPluginConfig,
      });
  ...
  it.skipIf(!pgReachable)(
    ...
    async () => {
      ...
      // #21: trustedProxyDepth comes from DI config, not process.env
      const { req, res } = makeReqRes({
        method: 'POST',
        ip: '8.8.8.8',
  ...
  it.skipIf(!pgReachable)('8. Unknown MerchantId: INSERT with NULL tenant, metric logged', async () => {
    ...
    // Override logger to capture warnings — must also re-expose vivaPluginConfig
    (req as unknown as { scope: { resolve: (k: string) => unknown } }).scope.resolve = (key: string) => {
      if (key === 'vivaPluginConfig') return test8Config;
      if (key === 'logger') {
        return {
          info: () => undefined,
