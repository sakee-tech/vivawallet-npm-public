...
async function runIsv(input) {
    ...
    try {
        ...
        const msg = err instanceof errors_1.VivaApiError ? err.message : String(err);
        console.error(`[viva-register-webhooks] Could not fetch the ISV webhook verification key ` +
            `(GET /isv/v1/webhooks/token): ${msg}`);
        return 2;
    }
    if (config.webhookVerificationKey !== vivaIssuedKey) {
        console.error(`[viva-register-webhooks] VIVA_WEBHOOK_VERIFICATION_KEY does not match the Viva-issued ISV key.\n` +
    ...
    if (opts.output !== 'json') {
        console.log('[viva-register-webhooks] ISV webhook verification key reconciled with Viva (GET /isv/v1/webhooks/token).');
    }
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    const desired = webhooks_1.ISV_EVENT_TYPE_IDS.map((eventTypeId) => ({
        eventTypeId,
        url: webhookUrl,
    ...
    if (opts.apply) {
        ...
        if (failures.length > 0) {
            console.error(`[viva-register-webhooks] ${applied} registered, ${failures.length} failed: ` +
