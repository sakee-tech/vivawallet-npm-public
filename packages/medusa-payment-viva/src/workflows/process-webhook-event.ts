...
import type { VivaWebhookEnvelope, TransactionEventData, AccountVerificationStatusChangedEventData } from '@sakeetech/viva-payments-core/types';
...
import type { VivaTransactionStatus } from '../models/viva-transaction.js';
...
export async function processWebhookEvent(
  ...
  if (isOnboardingEvent(eventTypeId) && effectiveMode === 'merchant') {
    logger.warn(
      `[viva] ${eventTypeId} received in merchant mode — ignored (eventId=${eventId})`,
    );
  ...
  if (!tenantId) {
    ...
    logger.warn(
      `[viva] processWebhookEvent: tenantId is null for eventId=${eventId} eventTypeId=${eventTypeId}. ` +
...
async function processOnboardingEvent(
  eventId: string,
  eventTypeId: number,
  envelope: VivaWebhookEnvelope,
  pool: pg.Pool,
  logger: ProcessWebhookContext['logger'],
): Promise<ProcessWebhookResult> {
  const eventData = envelope.EventData as AccountVerificationStatusChangedEventData & { ConnectedAccountId: string; Verified?: boolean };
  ...
  try {
    ...
    if (eventTypeId === 8194) {
      const verified = (eventData as { Verified?: boolean }).Verified;
      if (verified !== undefined) {
        await client.query(
          `UPDATE viva_tenant_merchant
             SET verification_status = $1, updated_at = now()
           WHERE connected_account_id = $2`,
          [verified ? 'verified' : 'unverified', connectedAccountId],
        );
      }
    }
...
async function processTransactionEvent(
  ...
  isvPayments: Payments,
  logger: ProcessWebhookContext['logger'],
  metrics: MetricsHook | undefined,
): Promise<ProcessWebhookResult> {
  const eventData = envelope.EventData as TransactionEventData;
  const vivaTransactionId = eventData.TransactionId as string;
  ...
  let liveStatusLetter: string;
  let liveRawPayload: Record<string, unknown>;
  let liveOrderCode: string | null = null;
  try {
    const live = await isvPayments.retrieveTransaction(
      vivaTransactionId as import('@sakeetech/viva-payments-core/types').TransactionId,
      merchantId ? { merchantId: merchantId as import('@sakeetech/viva-payments-core/types').MerchantId } : {},
    );
    liveStatusLetter = live.statusId;
    liveRawPayload = live as unknown as Record<string, unknown>;
    ...
    if (envOrderCodeStr && liveOrderCode && envOrderCodeStr !== liveOrderCode) {
      logger.warn(
        `[viva] orderCode mismatch — envelope=${envOrderCodeStr} live=${liveOrderCode} ` +
    ...
    logger.error(
      `[viva] retrieveTransaction failed for transactionId=${vivaTransactionId} eventId=${eventId}: ` +
  ...
  const { status: derivedNext, claimSubstate } = mapStatusLetter(
    liveStatusLetter as import('@sakeetech/viva-payments-core/types').VivaStatusLetter,
  );
  ...
  try {
    ...
    interface TxRow {
      ...
      claim_substate: string | null;
      raw_payload: Record<string, unknown> | null;
    }

    let txRow: TxRow | null = null;

    if (orderCode) {
      const result = await client.query<TxRow>(
        `SELECT viva_transaction_id, status, claim_substate, raw_payload
    ...
    if (!txRow) {
      ...
      logger.warn(
        `[viva] No viva_transaction row found for orderCode=${orderCode ?? 'null'} ` +
    ...
    if (!transition.ok) {
      await client.query('ROLLBACK');
      logger.warn(
        `[viva] Lattice rejected ${txRow.status} → ${derivedNext} ` +
    ...
    await client.query(
      `UPDATE viva_webhook_event SET processed_at = now() WHERE viva_webhook_event_id = $1`,
      [eventId],
    );
    ...
    logger.info(
      `[viva] Transaction event ${eventTypeId} applied: ` +
...
async function markProcessed(pool: pg.Pool, eventId: string): Promise<void> {
  const client = await pool.connect();
  try {
    await client.query(
      `UPDATE viva_webhook_event SET processed_at = now() WHERE viva_webhook_event_id = $1`,
