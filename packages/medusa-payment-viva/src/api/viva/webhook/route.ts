...
export const POST = async (
  ...
  const environment = (process.env['VIVA_ENVIRONMENT'] ?? 'demo') as 'demo' | 'production';
...
function parseExtraAllowlist(raw: string | undefined): readonly string[] {
  if (!raw) return [];
  return raw.split(',').map((s) => s.trim()).filter(Boolean);
}

function parseTrustedProxyDepth(raw: string | undefined): number {
  if (!raw) return 0;
