...
interface EcommerceBody extends BaseBody {
  domain?: unknown;
  pathSuccess?: unknown;
  pathFail?: unknown;
  isSecure?: unknown;
...
function parseBody(
  ...
  | { ok: true; kind: 'ecommerce'; input: Parameters<IsvSources['createEcommerceSource']>[0] }
  | { ok: true; kind: 'physical'; input: Parameters<IsvSources['createPhysicalSource']>[0] }
  | { ok: false; status: number; body: { error: string; message: string; code?: string } } {
  const body = (raw ?? {}) as EcommerceBody;
...
export const POST = async (
  ...
  // Config is resolved from the DI container (registered by the loader at
  // boot) rather than re-reading process.env at request time (#21).
  const config = resolveVivaConfig(req.scope);
  if (!config || config.mode !== 'isv') {
    // Defensive: reject404IfNotIsv already covered the isv check, but this
    // guard also handles the absent-config case and narrows the type to
    // VivaIsvConfig for the reseller access below.
  ...
  try {
    ...
    const account = await accounts.retrieveConnectedAccount(
      id as ConnectedAccountId,
    );
    ...
    const logger = req.scope?.resolve?.('logger') as
      ...
      | undefined;
    logger?.error?.(
      `[viva] POST /viva/admin/connected-accounts/:id/sources failed: ${err instanceof Error ? err.message : String(err)}`,
    );
