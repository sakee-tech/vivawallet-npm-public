...
import type { OAuth2ClientCredentialsStrategy } from '@sakeetech/viva-payments-core/auth';
...
export const GET = async (
  ...
  // Config is resolved from the DI container (registered at boot by the
  // plugin loader) rather than re-reading process.env at request time (#21).
  const pluginConfig = resolveVivaConfig(req.scope);
  const adminToken = pluginConfig?.adminToken;

  // If adminToken is not configured: always deny, document reason.
  if (!adminToken) {
    res.status(401).json({
      error: 'unauthorized',
  ...
  const environment = pluginConfig?.environment ?? 'demo';
  ...
  try {
    ...
    const strategy = req.scope?.resolve<OAuth2ClientCredentialsStrategy>(
