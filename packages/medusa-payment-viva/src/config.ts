...
export interface VivaCommonConfig {
  ...
  /**
   * Extra IP allowlist for the webhook endpoint (VIVA_WEBHOOK_IP_ALLOWLIST).
   * Comma-separated CIDR/IP strings appended to Viva's official allowlist.
   * #21: resolved from container, never read at request time.
   */
  webhookIpAllowlist?: readonly string[];

  /**
   * Number of X-Forwarded-For hops to trust (VIVA_TRUSTED_PROXY_DEPTH).
   * Default 0 = use socket IP only (safe for direct exposure).
   * #21: resolved from container, never read at request time.
   */
  trustedProxyDepth?: number;

  /**
   * Dev/CI escape hatch: bypass webhook IP allowlist check
   * (VIVA_WEBHOOK_IP_ALLOWLIST_BYPASS=true). Never set in production.
   * #21: resolved from container, never read at request time.
   */
  webhookIpAllowlistBypass?: boolean;
}
...
export function loadConfigFromEnv(
  ...
  if (mode === 'merchant') {
    if (!legacyMerchantId) {
      errors.push(
        'VIVA_MERCHANT_ID is required in merchant mode — Merchant Basic auth for refundPayment ' +
          '(DELETE /api/transactions/{id} on the legacy host).',
      );
    }
    if (!legacyApiKey) {
      errors.push(
        'VIVA_API_KEY is required in merchant mode — pair with VIVA_MERCHANT_ID for refund auth.',
      );
    }
  }
  ...
  // -------------------------------------------------------------------------
  // Webhook operational settings (#21 — all VIVA_-prefixed env must flow through
  // config so the loader registers them and callers resolve from req.scope).
  // -------------------------------------------------------------------------
  const webhookIpAllowlistRaw = env['VIVA_WEBHOOK_IP_ALLOWLIST'];
  const webhookIpAllowlist: readonly string[] = webhookIpAllowlistRaw
    ? webhookIpAllowlistRaw.split(',').map((s) => s.trim()).filter(Boolean)
    : [];

  const trustedProxyDepthRaw = env['VIVA_TRUSTED_PROXY_DEPTH'];
  let trustedProxyDepth = 0;
  if (trustedProxyDepthRaw) {
    const n = Number.parseInt(trustedProxyDepthRaw, 10);
    if (!Number.isNaN(n) && n >= 0) trustedProxyDepth = n;
  }

  const webhookIpAllowlistBypass = env['VIVA_WEBHOOK_IP_ALLOWLIST_BYPASS'] === 'true';
  ...
  const common: VivaCommonConfig = {
    ...
    ...(webhookIpAllowlist.length > 0 ? { webhookIpAllowlist } : {}),
    ...(trustedProxyDepth > 0 ? { trustedProxyDepth } : {}),
    ...(webhookIpAllowlistBypass ? { webhookIpAllowlistBypass } : {}),
  };
