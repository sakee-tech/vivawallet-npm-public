...
import type { OrderCode, MerchantId, MinorUnits, CurrencyCode, TransactionId } from '@sakeetech/viva-payments-core/types';
...
import type { VivaTransaction, VivaTransactionStatus } from './models/viva-transaction.js';
...
function buildCheckoutUrl(
  environment: VivaPluginConfig['environment'],
  orderCode: OrderCode,
): string {
  const baseUrl =
    environment === 'demo'
 ...
 */
const CURRENCY_ALPHA_TO_NUMERIC: Record<string, string> = {
  eur: '978',
  gbp: '826',
  usd: '840',
  pln: '985',
...
function toCurrencyCode(isoAlpha: string): CurrencyCode {
  ...
  if (!numeric) {
    ...
    return '978' as CurrencyCode;
  }
  return numeric as CurrencyCode;
}
 ...
 */
function toMedusaStatus(
  vivaStatus: VivaTransactionStatus,
): PaymentSessionStatus {
  switch (vivaStatus) {
    case 'authorized':
      return 'authorized';
 ...
 */
function toMedusaError(err: unknown): MedusaError {
 ...
 */
function buildLegacyClient(config: VivaPluginConfig): BasicAuthClient | undefined {
  // Merchant Basic client. Only built when the Merchant Basic creds are present
  // (merchant mode). In ISV mode they are empty and the refund builds a
  // reseller-variant client per-call instead, so return undefined here.
  if (!config.legacyMerchantId || !config.legacyApiKey) {
    return undefined;
  }
...
function buildPaymentsClient(
  ...
  httpClient: IsvHttpClient,
  legacyClient: BasicAuthClient | undefined,
): Payments {
  return new Payments({
    mode: config.mode,
    client: httpClient,
 ...
 */
function buildFastRefundClient(httpClient: IsvHttpClient): FastRefundClient {
 ...
 */
function effectiveMerchantId(
  config: VivaPluginConfig,
  tenantMerchantId: MerchantId | undefined,
): MerchantId {
  // In ISV mode the tenant resolver always returns a value before we get here.
  if (config.mode === 'isv' && tenantMerchantId) return tenantMerchantId;
  return config.legacyMerchantId as MerchantId;
}
 ...
 */
export class VivaPaymentProvider extends AbstractPaymentProvider<VivaPaymentProviderOptions> {
  static override readonly identifier = 'viva';
  ...
  constructor(
    ...
    options: VivaPaymentProviderOptions,
  ) {
    ...
    // @see docs/plans/multi-mode-v0.md §9
    this.isvPayments = buildPaymentsClient(
      options.config,
      httpClient,
      this.legacyClient,
    );
    ...
    this.em = (container['manager'] ?? container['entityManager']) as EntityManager;
  ...
  async initiatePayment(
    ...
    try {
      ...
      const contextMetadata = (input.data?.['cart_metadata'] ??
        input.data?.['metadata']) as Record<string, unknown> | null | undefined;

      const cartLike = {
        id: (input.data?.['cart_id'] as string | undefined) ?? '',
        metadata: contextMetadata ?? null,
        items: (input.data?.['items'] as Array<{ metadata?: Record<string, unknown> | null }> | undefined) ?? [],
      };
      ...
      // @see docs/plans/multi-mode-v0.md §9
      let vivaMerchantId: MerchantId | undefined;
      ...
      const repo = this.em.getRepository<VivaTransaction>('VivaTransaction');
      ...
      if (
        ...
        const redirectUrl = existing.viva_order_code
          ? buildCheckoutUrl(
              this.vivaConfig.environment,
              BigInt(existing.viva_order_code) as OrderCode,
            )
          : null;
        return {
          id: medusaPaymentId,
      ...
      const amountMinor = BigInt(Math.round(Number(input.amount) * 100)) as MinorUnits;
      ...
      const storedMerchantId: string | null = vivaMerchantId ?? null;
      ...
      if (!existing) {
        ...
        const entity = repo.create({
          ...
          status: 'initiated' as VivaTransactionStatus,
          claim_substate: null,
  ...
  async capturePayment(
    ...
    try {
      ...
      console.warn(
        `[viva] capturePayment called on transaction ${row.viva_transaction_id} ` +
  ...
  async refundPayment(
    ...
    try {
      ...
      if (row.status !== 'captured') {
        throw new VivaValidationError({
          message:
            `Cannot refund transaction '${row.viva_transaction_id}': ` +
      ...
      const refundAmountMinor = input.amount !== undefined && input.amount !== null
        ? (BigInt(Math.round(Number(input.amount) * 100)) as MinorUnits)
      ...
      if (refundAmountMinor > available) {
        throw new VivaValidationError({
          message:
            `Refund amount ${refundAmountMinor} exceeds available amount ${available} ` +
      ...
      const vivaTransactionId = (
        (row.raw_payload?.['TransactionId'] as string | undefined) ??
        (row.raw_payload?.['transactionId'] as string | undefined) ??
        row.viva_transaction_id
      ) as TransactionId;

      // Build idempotency key for the refund: combines refund id + amount to prevent duplicates
      const medusaRefundId = (input.data?.['medusa_refund_id'] as string | undefined) ?? medusaPaymentId;
      ...
      // @see docs/plans/multi-mode-v0.md §9
      const rowMerchantId = row.viva_merchant_id as MerchantId;

      if (this.vivaConfig.mode === 'isv') {
        // ISV refund authenticates as the Viva-issued reseller, scoped to the
        // connected merchant (DELETE /api/transactions/{id} on the legacy host).
        // The Merchant Basic client is inapplicable — build a reseller-variant
        // client with the connected merchant's UUID per-refund.
        // @see docs/internal/payment-isv-api.yaml:2650
        if (!this.vivaConfig.reseller) {
          throw new VivaValidationError({
            message:
              'ISV refund requires reseller credentials (VIVA_RESELLER_ID / ' +
              'VIVA_RESELLER_API_KEY). These are the Viva-issued Reseller ID/API key ' +
              '(demo and production pairs differ), distinct from the dashboard ISV credentials.',
          });
        }
        const resellerLegacyClient = new BasicAuthClient({
          authVariant: 'reseller',
          environment: this.vivaConfig.environment,
          resellerId: this.vivaConfig.reseller.resellerId,
          merchantId: rowMerchantId as string,
          resellerApiKey: this.vivaConfig.reseller.resellerApiKey,
        });
        await this.isvPayments.refundPayment(vivaTransactionId, {
          merchantId: rowMerchantId,
          amountMinor: refundAmountMinor,
          idempotencyKey: refundIdempotencyKey,
          legacyClient: resellerLegacyClient,
        });
      } else {
   ...
   */
  protected async refundPaymentMerchantMode(params: {
    ...
    try {
      ...
      console.warn(
        `[viva] retrieveTransaction failed for refund '${vivaTransactionId}': ` +
    ...
    await this.isvPayments.refundPayment(vivaTransactionId, {
      merchantId: this.vivaConfig.legacyMerchantId as MerchantId,
      amountMinor: refundAmountMinor,
      idempotencyKey: refundIdempotencyKey,
    });
  }
  ...
  async cancelPayment(
    ...
    try {
      ...
      if (!transition.ok) {
        throw new VivaValidationError({
          message:
            `Cannot cancel transaction '${row.viva_transaction_id}': ` +
      ...
      //   DELETE /api/orders/{oc} on the legacy host with Basic auth (the
      //   v2/OAuth2 route does not exist — verified 404; see Payments.cancelOrder).
      // - Merchant mode: uses the construction-time Merchant Basic legacyClient.
      // - ISV mode: build a Reseller-variant client scoped to the connected
      //   merchant per-call, exactly as the Standard refund path does.
      if (row.viva_order_code) {
        const cancelOpts: { merchantId?: MerchantId; legacyClient?: BasicAuthClient } = {};
        if (this.vivaConfig.mode === 'isv') {
          const rowMerchantId = row.viva_merchant_id as MerchantId;
          cancelOpts.merchantId = rowMerchantId;
          if (!this.vivaConfig.reseller) {
            throw new VivaValidationError({
              message:
                'ISV cancel requires reseller credentials (VIVA_RESELLER_ID / ' +
                'VIVA_RESELLER_API_KEY). These are the Viva-issued Reseller ID/API key ' +
                '(demo and production pairs differ), distinct from the dashboard ISV credentials. ' +
                'Cancel path: DELETE /api/orders/{oc} on the legacy host.',
            });
          }
          cancelOpts.legacyClient = new BasicAuthClient({
            authVariant: 'reseller',
            environment: this.vivaConfig.environment,
            resellerId: this.vivaConfig.reseller.resellerId,
            merchantId: rowMerchantId as string,
        ...
        await this.isvPayments.cancelOrder(
          BigInt(row.viva_order_code) as OrderCode,
          cancelOpts,
        );
      }
  ...
  async getWebhookActionAndData(
    ...
    try {
      ...
      const eventTypeId = eventData?.['EventTypeId'] as number | undefined;
      const innerEventData = eventData?.['EventData'] as Record<string, unknown> | undefined;
      const sessionId = (
        innerEventData?.['MerchantTrns'] ??
 ...
 */
function extractPaymentId(data: Record<string, unknown> | undefined): string | undefined {
  if (!data) return undefined;
  return (
    (data['medusa_payment_id'] as string | undefined) ??
