...
import type { VivaEventTypeId, VivaWebhookEnvelope } from '@sakeetech/viva-payments-core/types';
import { buildAuthStrategies } from '../resolvers/auth-strategy-factory.js';
import { resolveVivaConfig, type ResolvableScope } from '../container.js';
 ...
 */
export async function vivaWebhookEventSubscriber({
  event,
  container,
}: SubscriberArgs<VivaWebhookReceivedData>): Promise<void> {
  ...
  try {
    ...
    } | null) ?? { info: () => undefined, warn: () => undefined, error: () => undefined };
    ...
    try {
      ...
      let envelope: VivaWebhookEnvelope | null = null;
      try {
        const result = await client.query<{ raw_payload: VivaWebhookEnvelope }>(
          `SELECT raw_payload FROM viva_webhook_event WHERE viva_webhook_event_id = $1`,
          [data.eventId],
        );
      ...
      // Build Payments from config — resolved from the DI container
      // (registered once at boot by the plugin loader) rather than
      // re-reading process.env at event-dispatch time (#21).
      let isvPayments: Payments;
      let configMode: 'merchant' | 'isv' = 'isv';
      const vivaConfig = resolveVivaConfig(container as unknown as ResolvableScope);
      if (!vivaConfig) {
        logger.warn(`[viva] Subscriber: Viva plugin not configured for eventId=${data.eventId}. Skipping.`);
        return;
      }
      configMode = vivaConfig.mode;
      const authStrategies = buildAuthStrategies(vivaConfig);
      const httpClient = new IsvHttpClient({
        environment: vivaConfig.environment,
      ...
      await processWebhookEvent(
        {
          ...
          eventTypeId: data.eventTypeId as VivaEventTypeId,
          envelope,
      ...
      if (err instanceof VivaApiError) {
        ...
        logger.error(
          `[viva] Subscriber VivaApiError for eventId=${data.eventId}: ${err.message}. Rethrowing for retry.`,
        );
        throw err;
      }
      throw err;
...
// Medusa loads subscribers by their DEFAULT export (the handler). The named
// export above is retained for index.ts re-export and unit tests.
export default vivaWebhookEventSubscriber;
