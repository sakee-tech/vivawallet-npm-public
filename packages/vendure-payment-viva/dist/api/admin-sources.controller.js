...
let AdminSourcesController = class AdminSourcesController {
    ...
    async createSource(id, body, res) {
        ...
        try {
            ...
            if (parsed.kind === 'ecommerce' && response.sourceCode !== undefined) {
                try {
                    ...
                    if (channel) {
                        ...
                        await this.connectedAccounts.writeSourceCode(ctx, channel, response.sourceCode);
                        await this.connectedAccounts.assignVivaPaymentMethod(ctx, channel);
                    }
                    else {
                        core_1.Logger.warn(`[AdminSources] No channel with vivaAccountId=${id} — sourceCode=${String(response.sourceCode)} created at Viva but not persisted to a channel.`, constants_js_1.VIVA_LOG_CONTEXT);
                    }
                }
                catch (persistErr) {
                    core_1.Logger.error(`[AdminSources] sourceCode=${String(response.sourceCode)} created at Viva but ` +
