...
const ip_allowlist_js_1 = require("../util/ip-allowlist.js");
const queue_names_js_1 = require("../jobs/queue-names.js");
// ---------------------------------------------------------------------------
// Controller
// ---------------------------------------------------------------------------
let WebhookController = class WebhookController {
    ...
    async onModuleInit() {
        ...
        this.webhookQueue = await this.jobQueueService.createQueue({
            name: queue_names_js_1.VIVA_WEBHOOK_QUEUE,
            process: async (_job) => {
    ...
    handleVerification(res) {
        ...
        if (!key) {
            core_1.Logger.warn('webhookVerificationKey is not configured — returning 503. Set VIVA_WEBHOOK_VERIFICATION_KEY.', constants_js_1.VIVA_LOG_CONTEXT);
    ...
    async handleEvent(req, res) {
        ...
        const sourceIp = (0, ip_allowlist_js_1.getSourceIp)(req, this.options.trustedProxyDepth ?? 0);
        const allowlist = this.options.webhookIpAllowlist;
        if (!(0, ip_allowlist_js_1.isIpAllowed)(sourceIp, allowlist)) {
            core_1.Logger.warn(`Webhook POST rejected: source IP ${sourceIp} not in allowlist.`, constants_js_1.VIVA_LOG_CONTEXT);
        ...
        if (!body || typeof body.MessageId !== 'string' || !body.MessageId) {
            core_1.Logger.warn('Webhook POST: missing or invalid MessageId.', constants_js_1.VIVA_LOG_CONTEXT);
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'malformed envelope' }));
            return;
        }
        if (typeof body.EventTypeId !== 'number' || !body.EventTypeId) {
            core_1.Logger.warn(`Webhook POST: missing or invalid EventTypeId for MessageId=${body.MessageId}.`, constants_js_1.VIVA_LOG_CONTEXT);
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'malformed envelope' }));
            return;
        }
        ...
        const insertResult = await repo
            .createQueryBuilder()
            .insert()
            .into(viva_webhook_event_entity_js_1.VivaWebhookEvent)
            .values({
            messageId,
            eventTypeId,
            ...
            accountId: eventData['ConnectedAccountId'] ?? null,
        ...
        // ------------------------------------------------------------------
        if (rowsInserted === 0) {
            core_1.Logger.debug(`Webhook POST: duplicate MessageId=${messageId} (replay) — skipping enqueue.`, constants_js_1.VIVA_LOG_CONTEXT);
            res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        try {
            ...
            core_1.Logger.warn(`Webhook POST: job enqueue failed for MessageId=${messageId}. Row persisted; worker will retry. Error: ${String(err)}`, constants_js_1.VIVA_LOG_CONTEXT);
        }
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: true }));
    }
...
exports.WebhookController = WebhookController = __decorate([
    (0, common_1.Controller)('viva/webhook'),
