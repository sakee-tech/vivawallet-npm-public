...
let AdminOnboardingController = class AdminOnboardingController {
    ...
    isvOptions() {
        if (this.options.mode !== 'isv') {
            throw new errors_1.VivaModeMismatchError({
    ...
    sendVivaError(res, err) {
        if (err instanceof errors_1.VivaAuthError || (err instanceof errors_1.VivaApiError && err.httpStatus >= 500)) {
            const pluginErr = error_envelope_js_1.VivaPluginError.authDown(err instanceof Error ? err.message : 'Viva service unavailable', err);
        ...
        if (err instanceof errors_1.VivaApiError) {
            ...
            const pluginErr = error_envelope_js_1.VivaPluginError.apiError(pluginErrOpts);
        ...
        const pluginErr = error_envelope_js_1.VivaPluginError.internalError(err instanceof Error ? err.message : 'Unexpected error', err);
    ...
    async initiateOnboarding(body, res) {
        ...
        if (!channelId) {
            const err = error_envelope_js_1.VivaPluginError.channelMisconfigured('channelId is required');
        ...
        if (!channel) {
            const err = error_envelope_js_1.VivaPluginError.channelMisconfigured(`Channel ${channelId} not found`);
        ...
        if (!seller) {
            const err = error_envelope_js_1.VivaPluginError.channelMisconfigured('Channel must have a Seller before onboarding');
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify(err.toJSON()));
            return;
        }
        ...
        if (existingAccountId) {
            const err = error_envelope_js_1.VivaPluginError.alreadyOnboarded(existingAccountId);
        ...
        if (!email) {
            const err = error_envelope_js_1.VivaPluginError.channelMisconfigured('Seller must have an email (customFields.contactEmail, customFields.email, or seller.email) — or pass `overrides.email`.');
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify(err.toJSON()));
            return;
        }
        if (!returnUrl) {
            const err = error_envelope_js_1.VivaPluginError.channelMisconfigured('`onboardingReturnUrl` is required in plugin options (or pass `overrides.returnUrl`).');
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify(err.toJSON()));
            return;
        }
        ...
        core_1.Logger.info(`[AdminOnboarding] Initiating onboarding for channel ${String(channel.id)}.`, constants_js_1.VIVA_LOG_CONTEXT);
        ...
        try {
            ...
            core_1.Logger.error(`[AdminOnboarding] IsvAccounts.createConnectedAccount failed for channel ${String(channel.id)}: ${String(err)}`, constants_js_1.VIVA_LOG_CONTEXT);
        ...
        try {
            ...
            core_1.Logger.error(`[AdminOnboarding] Failed to write vivaAccountId to channel ${String(channel.id)}: ${String(err)}`, constants_js_1.VIVA_LOG_CONTEXT);
            // accountId was created at Viva — still return it so the admin can record it
            core_1.Logger.warn(`[AdminOnboarding] accountId=${accountId} was created at Viva but could not be persisted locally. Manual recovery required.`, constants_js_1.VIVA_LOG_CONTEXT);
        }
        // 7. Return response
        ...
        if (accountId) {
            ...
            if (cached !== undefined) {
                ...
                try {
                    ...
                    core_1.Logger.warn(`[AdminOnboarding] IsvAccounts.retrieveConnectedAccount failed for accountId=${accountId}: ${String(err)}`, constants_js_1.VIVA_LOG_CONTEXT);
                }
            }
        }
        const payload = {
    ...
    async reconcile(channelId, res) {
        ...
        if (!accountId) {
            const err = error_envelope_js_1.VivaPluginError.channelMisconfigured('Channel does not have a vivaAccountId. Initiate onboarding first.');
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify(err.toJSON()));
            return;
        }
        ...
        try {
            ...
            core_1.Logger.error(`[AdminOnboarding] reconcile: IsvAccounts.retrieveConnectedAccount failed for channel ${channelId}: ${String(err)}`, constants_js_1.VIVA_LOG_CONTEXT);
            this.sendVivaError(res, err);
            return;
        }
        ...
        if (isVerified && merchantId) {
            ...
            try {
                ...
                core_1.Logger.info(`[AdminOnboarding] reconcile: wrote merchantId=${merchantId} and flipped payoutsEnabled for channel ${channelId}.`, constants_js_1.VIVA_LOG_CONTEXT);
            }
            catch (err) {
                core_1.Logger.error(`[AdminOnboarding] reconcile: failed to write channel fields for channel ${channelId}: ${String(err)}`, constants_js_1.VIVA_LOG_CONTEXT);
                const pluginErr = error_envelope_js_1.VivaPluginError.internalError('Failed to persist reconciliation data', err);
                res.writeHead(500, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify(pluginErr.toJSON()));
                return;
            }
...
__decorate([
    (0, common_1.Post)(':channelId/reconcile'),
    (0, core_1.Allow)(core_1.Permission.SuperAdmin),
    __param(0, (0, common_1.Param)('channelId')),
    __param(1, (0, common_1.Res)()),
    __metadata("design:type", Function),
