...
let StateMachineService = class StateMachineService {
    ...
    /**
     * Load a VivaTransaction row by its globally-unique `vivaOrderCode`.
     * Returns null if not found. Used by the `orderByVivaReturn` Shop API query to
     * resolve a Smart Checkout return (?s=<vivaOrderCode>) to its Vendure order via
     * the unique index `idx_viva_transaction_order_code` — no ISV API call.
     */
    async getVivaTransactionByOrderCode(ctx, vivaOrderCode) {
        const repo = this.connection.getRepository(ctx, viva_transaction_entity_js_1.VivaTransaction);
        return repo.findOne({ where: { vivaOrderCode } });
    }
    /**
     ...
     */
    async reconcilePaymentId(ctx, vivaOrderCode, realPaymentId) {
        const repo = this.connection.getRepository(ctx, viva_transaction_entity_js_1.VivaTransaction);
        const result = await repo.update({ vivaOrderCode }, { paymentId: realPaymentId });
