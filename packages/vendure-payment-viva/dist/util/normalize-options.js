...
export function _resetInitNoticesForTesting() {
...
export function normalizePluginOptions(input) {
    const errors = [];
    // -------------------------------------------------------------------------
    ...
    if (!input.legacyMerchantId) {
        errors.push('legacyMerchantId is required — needed for createRefund (Viva legacy Basic-auth API).');
    }
    if (!input.legacyApiKey) {
        errors.push('legacyApiKey is required — needed for createRefund (Viva legacy Basic-auth API).');
    }
