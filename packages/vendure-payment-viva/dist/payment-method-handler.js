...
exports.vivaPaymentMethodHandler = new core_1.PaymentMethodHandler({
    ...
    async cancelPayment(ctx, _order, payment, _args) {
        ...
        if (row && row.status === 'captured' && payment.state !== 'Settled') {
        ...
        else if ((0, cancel_guard_js_1.classifyCancel)(payment.state) !== 'proceed') {
            ...
            throw error_envelope_js_1.VivaPluginError.paymentNotCancellable(`Payment is already in terminal state: ${row?.status ?? payment.state}.`);
        }
