...
exports.vivaPaymentMethodHandler = new core_1.PaymentMethodHandler({
    ...
    async cancelPayment(ctx, _order, payment, _args) {
        ...
        const vivaOrderCode = row?.vivaOrderCode ?? paymentMeta['vivaOrderCode'];
        if (!vivaOrderCode) {
        ...
        if (options.mode === 'isv') {
            ...
            merchantId =
                (resolveMerchantId(options, ctx) ??
                    rowMeta['vivaMerchantId'] ??
                    paymentMeta['vivaMerchantId']);
            if (!merchantId) {
                throw error_envelope_js_1.VivaPluginError.channelMisconfigured();
            }
        }
        ...
        if (row && row.status === 'captured' && payment.state !== 'Settled') {
            ...
            if (txnId) {
                try {
                    ...
                    const tx = await isvPayments.retrieveTransaction(txnId, opts);
            ...
            if (reallyCaptured) {
                ...
                core_1.Logger.warn(`[cancelPayment] Payment ${String(payment.id)} row is 'captured' and Viva confirms ` +
                    `transaction ${String(txnId)} is captured, but the Vendure Payment was still ` +
                    `'${payment.state}'. Settling it to resolve the desync (#26).`, constants_js_1.VIVA_LOG_CONTEXT);
                ...
                throw error_envelope_js_1.VivaPluginError.paymentNotCancellable('Payment was already captured at Viva and has now been settled — order is paid, not cancellable.');
            }
            ...
            core_1.Logger.warn(`[cancelPayment] Payment ${String(payment.id)} row is 'captured' but Viva does NOT ` +
                `confirm capture (txn ${String(txnId ?? 'none')}); the row was mis-marked. Freeing the ` +
                `payment so the order can be retried (#26).`, constants_js_1.VIVA_LOG_CONTEXT);
        }
        else if ((0, cancel_guard_js_1.shouldRefuseCancel)(row?.status, payment.state)) {
            // Genuinely terminal (settled-captured, refunded, failed, cancelled) — refuse.
            // Same shared predicate the resolver pre-guard uses, so the two layers can
            // never drift (the #26 regression). The captured/unsettled desync is handled
            // by the reconcile branch above and never reaches here.
        ...
        if (options.mode === 'isv') {
            if (!options.reseller) {
                throw error_envelope_js_1.VivaPluginError.channelMisconfigured('ISV cancel requires reseller credentials (options.reseller = { resellerId, ' +
            ...
            cancelOpts.legacyClient = getResellerLegacyClient(options, merchantId);
        }
        try {
            ...
            const detail = err instanceof errors_1.VivaApiError
                ...
                : err instanceof Error
                    ? err.message
                    : String(err);
            core_1.Logger.warn(`[cancelPayment] Viva cancelOrder for order ${vivaOrderCode} failed non-retryably (${detail}) — ` +
