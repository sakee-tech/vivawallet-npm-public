 ...
 */
function isOrderAlreadyGoneVivaError(err) {
    return err instanceof errors_1.VivaApiError && err.httpStatus === 404;
}
// ---------------------------------------------------------------------------
...
exports.vivaPaymentMethodHandler = new core_1.PaymentMethodHandler({
    ...
    async cancelPayment(ctx, _order, payment, _args) {
        ...
        try {
            ...
            core_1.Logger.info(`[cancelPayment] Viva order ${row.vivaOrderCode} already gone (HTTP 404) — cancelling the local payment so the order can be retried.`, constants_js_1.VIVA_LOG_CONTEXT);
        }
