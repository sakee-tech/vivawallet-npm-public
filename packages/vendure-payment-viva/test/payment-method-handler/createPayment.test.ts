...
describe('createPayment — idempotency', () => {
  it('stamps expiresAt on the row metadata at mint time (~1800s out)', async () => {
    const pool = agent.get(DEMO_API_HOST);
    const { strategy, stateMachine } = setupHandler(agent);
    await seedToken(strategy);
    ...
    const expiresAt = stateMachine.rows.get('1:15')?.metadata?.['expiresAt'] as number;
    expect(typeof expiresAt).toBe('number');
    // Viva default paymentTimeout is 1800s; allow slack for test execution time.
    expect(expiresAt).toBeGreaterThanOrEqual(before + 1700 * 1000);
    expect(expiresAt).toBeLessThanOrEqual(Date.now() + 1800 * 1000);
  });

  it('second createPayment with a live cached order returns the cached redirect URL without calling Viva', async () => {
    const pool = agent.get(DEMO_API_HOST);
    const { strategy, stateMachine } = setupHandler(agent);
    await seedToken(strategy);

    // First call — Viva responds
    mockVivaOrderResponse(pool, 555n);
    const ctx = makeCtx();
    const order = makeOrder({ id: 20, currencyCode: 'GBP' });
    await (vivaPaymentMethodHandler as any).createPaymentFn(ctx, order, 1000, {}, {});

    // Persisted state: orderCode + a clearly-future expiry (still live).
    const rowKey = '1:20';
    ...
    // Second call — should NOT hit Viva.
    let callCount = 0;
    ...
    expect(result2.metadata['redirectUrl']).toBe('https://demo.vivapayments.com/web/checkout?ref=555');
    // #19: the idempotency path must also surface the redirect in the `public`
    // slice — Vendure's Shop API only exposes `public.*` to customers, so a
    // missing public.redirectUrl strands the storefront with no Smart Checkout.
  ...
  it('expired cached order → mints a FRESH order instead of returning the dead redirect', async () => {
    const pool = agent.get(DEMO_API_HOST);
    const { strategy, stateMachine } = setupHandler(agent);
    await seedToken(strategy);
    ...
    // Persisted state: orderCode + an expiry in the past (Viva order is dead).
    const rowKey = '1:21';
    const row = stateMachine.rows.get(rowKey)!;
    stateMachine.rows.set(rowKey, {
      ...row,
    ...
    // Second call — MUST hit Viva for a fresh order.
    let callCount = 0;
    pool.intercept({ path: /\/checkout\/v2\/(isv\/)?orders/, method: 'POST' }).reply(200, () => {
      callCount++;
    ...
    expect(result2.metadata['vivaOrderCode']).toBe('777');
    expect(result2.metadata['redirectUrl']).toContain('ref=777');
    expect(result2.metadata['redirectUrl']).not.toContain('555');
    // Row's expiry was refreshed forward.
    expect(stateMachine.rows.get(rowKey)?.metadata?.['expiresAt']).toBeGreaterThan(Date.now());
  });

  it('within the expiry skew window → treats as expired and re-mints (no near-dead redirect)', async () => {
    const pool = agent.get(DEMO_API_HOST);
    const { strategy, stateMachine } = setupHandler(agent);
    await seedToken(strategy);
    ...
    mockVivaOrderResponse(pool, 555n);
    const ctx = makeCtx();
    const order = makeOrder({ id: 22, currencyCode: 'GBP' });
    await (vivaPaymentMethodHandler as any).createPaymentFn(ctx, order, 1000, {}, {});
    ...
    stateMachine.rows.set(rowKey, {
      ...
      metadata: {
        ...row.metadata as object,
        redirectUrl: 'https://demo.vivapayments.com/web/checkout?ref=555',
        // 30s out — inside the 60s skew, so it must NOT be reused.
    ...
    const result2 = await (vivaPaymentMethodHandler as any).createPaymentFn(ctx, order, 1000, {}, {});
    expect(callCount).toBe(1);
    expect(result2.metadata['vivaOrderCode']).toBe('888');
  });

  it('legacy row without expiresAt → treats as expired and re-mints', async () => {
    const pool = agent.get(DEMO_API_HOST);
    const { strategy, stateMachine } = setupHandler(agent);
    await seedToken(strategy);
    ...
    mockVivaOrderResponse(pool, 555n);
    const ctx = makeCtx();
    const order = makeOrder({ id: 23, currencyCode: 'GBP' });
    await (vivaPaymentMethodHandler as any).createPaymentFn(ctx, order, 1000, {}, {});

    // Simulate a row persisted before this fix: orderCode present, NO expiresAt.
    const rowKey = '1:23';
    const row = stateMachine.rows.get(rowKey)!;
    const legacyMeta = { ...(row.metadata as Record<string, unknown>) };
    delete legacyMeta['expiresAt'];
    legacyMeta['redirectUrl'] = 'https://demo.vivapayments.com/web/checkout?ref=555';
    stateMachine.rows.set(rowKey, { ...row, vivaOrderCode: '555', metadata: legacyMeta });

    let callCount = 0;
    pool.intercept({ path: /\/checkout\/v2\/(isv\/)?orders/, method: 'POST' }).reply(200, () => {
      callCount++;
      return { orderCode: 666 };
    }, { headers: { 'content-type': 'application/json' } });

    const result2 = await (vivaPaymentMethodHandler as any).createPaymentFn(ctx, order, 1000, {}, {});
    expect(callCount).toBe(1);
    expect(result2.metadata['vivaOrderCode']).toBe('666');
  });
});
