...
describe('createPayment — idempotency', () => {
  it('second createPayment with existing row+orderCode returns cached redirect URL without calling Viva', async () => {
    const pool = agent.get(DEMO_API_HOST);
    const { strategy, stateMachine } = setupHandler(agent);
    await seedToken(strategy);
    ...
    // Set vivaOrderCode on the row to simulate persisted state
    const rowKey = '1:20';
    const row = stateMachine.rows.get(rowKey)!;
    stateMachine.rows.set(rowKey, {
      ...row,
      vivaOrderCode: '555',
      metadata: { ...row.metadata as object, redirectUrl: 'https://demo.vivapayments.com/web/checkout?ref=555' },
    });

    // Second call — should NOT hit Viva (no more mock intercepts registered)
    let callCount = 0;
