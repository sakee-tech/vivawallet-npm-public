...
describe('PROBE: createPayment → Viva createOrder request (ISV)', () => {
  it('emits amount, currencyCode 826, Default source, and the 99p isvAmount', async () => {
    ...
    // the real adapter), Default source, the platform fee as a bigint, plus the
    // merchant/customer reference (#11). makeOrder() has no customer here, so
    // only merchantTrns + customerTrns are present (customer fields omitted).
    expect(body).toEqual({
      amount: BigInt(ORDER_AMOUNT_MINOR),
      currencyCode: '826',
      sourceCode: 'Default',
      merchantTrns: 'ORDER-013',
      customerTrns: 'Order ORDER-013',
  ...
  // ---------------------------------------------------------------------------
  // #11 — customer identity + merchant reference
  // ---------------------------------------------------------------------------

  it('#11 — forwards customerEmail/customerFullName/customerPhone when a customer is set', async () => {
    const createOrder = vi.fn().mockResolvedValue({ orderCode: 42n });
    injectWith(createOrder);
    ...
    await call(makeCtx(), order, ORDER_AMOUNT_MINOR);

    const [body] = createOrder.mock.calls[0];
    expect(body.merchantTrns).toBe('ORDER-013'); // merchant ref ← order.code
    expect(body.customerTrns).toBe('Order ORDER-013');
    expect(body.customerEmail).toBe('buyer@example.com');
    expect(body.customerFullName).toBe('Ada Lovelace');
    expect(body.customerPhone).toBe('+447700900123');
  });

  it('#11 — omits customer fields entirely when no customer relation is present', async () => {
    const createOrder = vi.fn().mockResolvedValue({ orderCode: 43n });
    injectWith(createOrder);

    await call(makeCtx(), makeOrder(), ORDER_AMOUNT_MINOR);

    const [body] = createOrder.mock.calls[0];
    // merchant/customer reference always sent…
    expect(body.merchantTrns).toBe('ORDER-013');
    expect(body.customerTrns).toBe('Order ORDER-013');
    // …but no customer identity when the order has no customer.
    expect(body).not.toHaveProperty('customerEmail');
    expect(body).not.toHaveProperty('customerFullName');
    expect(body).not.toHaveProperty('customerPhone');
  });

  it('#11 — a partial customer (email only) sends customerEmail, omits name/phone', async () => {
    const createOrder = vi.fn().mockResolvedValue({ orderCode: 44n });
    injectWith(createOrder);

    await call(makeCtx(), makeOrder({ emailAddress: 'only@example.com' }), ORDER_AMOUNT_MINOR);

    const [body] = createOrder.mock.calls[0];
    expect(body.customerEmail).toBe('only@example.com');
    expect(body).not.toHaveProperty('customerFullName');
    expect(body).not.toHaveProperty('customerPhone');
  });

  it('#11 — resolveCustomerTrns overrides the default customerTrns', async () => {
    const createOrder = vi.fn().mockResolvedValue({ orderCode: 45n });
    injectWith(createOrder, {
      resolveCustomerTrns: (o: any) => `Acme Store — order ${o.code}`,
    });

    await call(makeCtx(), makeOrder(), ORDER_AMOUNT_MINOR);

    const [body] = createOrder.mock.calls[0];
    expect(body.customerTrns).toBe('Acme Store — order ORDER-013');
    expect(body.merchantTrns).toBe('ORDER-013'); // merchantTrns is NOT configurable
  });

  it('#11 — clamps merchantTrns ≤50 and customerTrns ≤255', async () => {
    const createOrder = vi.fn().mockResolvedValue({ orderCode: 46n });
    const longCode = 'X'.repeat(80);
    injectWith(createOrder, {
      resolveCustomerTrns: () => 'Y'.repeat(400),
    });

    // Override order.code with an over-long value.
    const order = { ...makeOrder(), code: longCode };
    await call(makeCtx(), order, ORDER_AMOUNT_MINOR);

    const [body] = createOrder.mock.calls[0];
    expect(body.merchantTrns).toHaveLength(50);
    expect(body.customerTrns).toHaveLength(255);
  });
