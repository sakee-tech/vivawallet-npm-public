...
export function makeOptions(overrides: Partial<VivaPaymentPluginOptions> = {}): VivaPaymentPluginOptions {
  return {
    ...
    // Merchant Basic creds (merchant-mode refund/webhook-key). Present here for
    // merchant-mode tests that override `mode`.
    legacyMerchantId: 'test-legacy-merchant-uuid',
    legacyApiKey: 'test-legacy-api-key',
    // ISV refund (DELETE /api/transactions/{id}) authenticates with the
    // Viva-issued Reseller pair scoped to the connected merchant.
    reseller: {
      resellerId: 'test-reseller-id',
      merchantId: 'test-reseller-merchant-uuid',
      resellerApiKey: 'test-reseller-api-key',
    },
