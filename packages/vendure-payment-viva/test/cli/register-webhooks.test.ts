...
describe('register-webhooks CLI', () => {
  beforeEach(() => {
    ...
    isvMock.getVerificationKey.mockResolvedValue({ key: 'mock-key' });
  ...
  it('dry-run surfaces generatedVerificationKey when env key absent', async () => {
    ...
    try {
      const opts: RunOptions = { dryRun: true, apply: false, output: 'text' };
      await run(opts, envWithoutKey);
    } finally {
      console.warn = origWarn;
    }
    expect(warns.some((w) => w.includes('VIVA_WEBHOOK_VERIFICATION_KEY'))).toBe(true);
    expect(warns.some((w) => /[0-9a-f-]{36}/.test(w))).toBe(true);
  });
