...
describe('register-webhooks CLI', () => {
  ...
  it('dry-run: returns exit 1 because every run emits 5 REGISTER actions (no list endpoint)', async () => {
  ...
  it('apply happy path: registers the 5 ISV-registerable event types (no 4865), exit 0', async () => {
    const opts: RunOptions = { dryRun: false, apply: true, output: 'text' };
    const code = await run(opts, BASE_ENV);
    expect(code).toBe(0);
    expect(isvMock.registerWebhook).toHaveBeenCalledTimes(5);
    const registeredIds = isvMock.registerWebhook.mock.calls.map(
      (call) => (call[0] as { eventTypeId: number }).eventTypeId,
    );
    // 4865 (Order Updated) excluded — not ISV-registerable (#18).
    expect(registeredIds.sort((a, b) => a - b)).toEqual([1796, 1797, 1798, 8193, 8194]);
  });

  it('continue-on-error: one event rejected → others still registered, exit 2 (#18)', async () => {
    // Reject only 8193; every other event must still be attempted.
    isvMock.registerWebhook.mockImplementation(async (req: { eventTypeId: number }) => {
      if (req.eventTypeId === 8193) {
        throw Object.assign(new VivaApiError('invalid', 400, undefined), { httpStatus: 400 });
      }
    });
    ...
    // All 5 attempted despite the 8193 rejection (the bug was aborting early).
    expect(isvMock.registerWebhook).toHaveBeenCalledTimes(5);
    const attempted = isvMock.registerWebhook.mock.calls.map(
      (c) => (c[0] as { eventTypeId: number }).eventTypeId,
    );
    expect(attempted).toContain(8194);
  });
