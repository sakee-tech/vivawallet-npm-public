...
describe('VivaShopApiResolver.cancelPayment', () => {
  ...
  // #27 REGRESSION GUARD — a diverged `cancelled` ROW must NOT brick a Payment
  // that is still `Created`. The authoritative Payment.state says `Created` =
  // cancellable, so the resolver MUST fall through to PaymentService.cancelPayment
  // and self-heal, NOT refuse on the row status. The previous test asserted the
  // opposite (cancelled row → always refused) and so GREEN-LIT the 0.3.3 brick:
  // once a half-applied cancel left `row=cancelled / payment=Created`, every retry
  // was refused forever. This is exactly the case that regressed.
  it('#27: cancelled row + Payment still Created → delegates to PaymentService.cancelPayment (self-heals, no refusal)', async () => {
    const order = makeOrder({ id: 100, customerUserId: 42, state: 'ArrangingPayment' });
    const payment = makePayment({ id: 99, orderId: 100, state: 'Created' });
    const ctx = makeCtx({ activeUserId: 42 });
  ...
  // #27 idempotency — once the Payment itself is genuinely `Cancelled` (a prior
  // call, or the winner of the concurrent cancel-return render storm), a repeat
  // cancel must return the Order, NOT an error, and must NOT re-void Viva.
  it('#27: Payment already Cancelled → idempotent success (Order), no PaymentService.cancelPayment call', async () => {
    const order = makeOrder({ id: 100, customerUserId: 42, state: 'AddingItems' });
    const payment = makePayment({ id: 99, orderId: 100, state: 'Cancelled' });
    const ctx = makeCtx({ activeUserId: 42 });
  ...
  // A genuinely Settled Payment is paid — refuse the cancel (operator must refund).
  it('Payment Settled → refused (use refund), no PaymentService.cancelPayment call', async () => {
    const order = makeOrder({ id: 100, customerUserId: 42 });
    const payment = makePayment({ id: 99, orderId: 100, state: 'Settled' });
    const ctx = makeCtx({ activeUserId: 42 });

    const deps = buildDeps({ payment, order, vivaRow: makeVivaRow({ status: 'captured' }) });
    const resolver = buildResolver(deps);
  ...
  // #26 REGRESSION GUARD — the Step-6 pre-guard must NOT shadow the handler's
  // reconcile for the captured/unsettled desync. A `captured` row whose Payment
  // never settled is reconcilable, not terminal: the resolver must fall through
  // to PaymentService.cancelPayment (Step 7), the only path that can clear it.
  // The previous test asserted the opposite (captured → always refused) and so
  // GREEN-LIT the 0.3.2 brick. This is exactly the case that regressed.
  it('#26: captured row + Payment NOT Settled → delegates to PaymentService.cancelPayment (no Step-6 refusal)', async () => {
    const order = makeOrder({ id: 100, customerUserId: 42, state: 'ArrangingPayment' });
    const payment = makePayment({ id: 99, orderId: 100, state: 'Created' });
    const ctx = makeCtx({ activeUserId: 42 });
    ...
    // Fell through Step 6 → reached Step 7. The handler reconcile is now reachable.
    expect(deps.paymentService.cancelPayment).toHaveBeenCalledOnce();
    expect(deps.paymentService.cancelPayment).toHaveBeenCalledWith(ctx, payment.id);
    expect(result.__typename).toBe('Order');
  });

  it('#26: captured row + Payment Settled → genuinely terminal, refused (cancelPayment NOT called)', async () => {
    const order = makeOrder({ id: 100, customerUserId: 42 });
    const payment = makePayment({ id: 99, orderId: 100, state: 'Settled' });
    const ctx = makeCtx({ activeUserId: 42 });
    ...
    expect(result.__typename).toBe('CancelPaymentError');
    expect(result.errorCode).toBe('VIVA_PAYMENT_NOT_CANCELLABLE');
    // Refusal now keys off the authoritative Payment.state (`Settled` = paid),
    // not the row status — the message reflects that.
    expect(result.message).toContain('settled');
    expect(deps.paymentService.cancelPayment).not.toHaveBeenCalled();
  });
