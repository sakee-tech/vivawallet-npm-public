 ...
 * Mocks: OrderService, PaymentService, TransactionalConnection, StateMachineService,
 *        vivaPaymentMethodHandler.cancelPaymentFn.
 *
 * Covers:
...
import { vivaPaymentMethodHandler } from '../../src/payment-method-handler.js';
...
function buildDeps(overrides: Partial<{
  ...
  // Spy on the handler's cancelPaymentFn
  vi.spyOn(vivaPaymentMethodHandler as any, 'cancelPaymentFn').mockImplementation(async () => {
...
describe('VivaShopApiResolver.cancelPayment', () => {
  ...
  it('happy path: customer-owned order → returns Order', async () => {
    ...
    expect((vivaPaymentMethodHandler as any).cancelPaymentFn).toHaveBeenCalledOnce();
  });
  ...
  it('wrong customer: returns CancelPaymentError AUTHORIZATION_FAILED', async () => {
    ...
    expect((vivaPaymentMethodHandler as any).cancelPaymentFn).not.toHaveBeenCalled();
  });
  ...
  it('idempotent re-call: order already AddingItems → returns Order (no error)', async () => {
    ...
    // cancelPaymentFn still called (Viva void may be needed for the row)
    expect((vivaPaymentMethodHandler as any).cancelPaymentFn).toHaveBeenCalledOnce();
