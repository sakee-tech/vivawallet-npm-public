...
function buildController(
  ...
  const connectedAccounts = {
    findChannelByAccountId: vi.fn().mockResolvedValue(channel),
    writeSourceCode: vi.fn().mockResolvedValue(undefined),
    assignVivaPaymentMethod: vi.fn().mockResolvedValue(true),
  };
  const requestContextService = { create: vi.fn().mockResolvedValue({ ctx: true }) };
  const ctrl = new AdminSourcesController(
    options,
    oauth2 as any,
  ...
  return { ctrl, retrieve, connectedAccounts, requestContextService, channel };
}
...
describe('AdminSourcesController — POST /viva/admin/connected-accounts/:id/sources', () => {
  ...
  it('3. ISV account not verified → 409 VIVA_ACCOUNT_NOT_VERIFIED', async () => {
    ...
    await ctrl.createSource(
      'acc_test_001',
      { kind: 'ecommerce', domain: 'example.com', pathSuccess: '/ok', pathFail: '/no', name: 'Web', sourceCode: '1234' },
      res,
    );
  ...
  it('4. ecommerce 201 + createEcommerceSource called with body input', async () => {
    ...
    // sourceCode comes from the request input, NOT from the (non-existent) response.
    createEcommerceSourceMock.mockResolvedValueOnce(undefined);
    const { ctrl, connectedAccounts, channel } = buildController(makeIsvOptions());
    const res = makeMockResponse();
    await ctrl.createSource(
      'acc_test_001',
      {
    ...
    const body = JSON.parse(res._body);
    // sourceCode in response comes from the request input (parsed.input.sourceCode),
    // not from a response field (defect #24 fix).
    ...
    expect(arg.sourceCode).toBe('1234');
    // Closes the onboarding loop: persists sourceCode to the channel resolved
    // by vivaAccountId, then assigns the Viva PaymentMethod.
    expect(connectedAccounts.findChannelByAccountId).toHaveBeenCalledWith('acc_test_001');
    expect(connectedAccounts.writeSourceCode).toHaveBeenCalledWith(
      expect.anything(),
      channel,
      '1234',
    );
    expect(connectedAccounts.assignVivaPaymentMethod).toHaveBeenCalledWith(
      expect.anything(),
      channel,
    );
  });
  ...
  it('5. physical 201 + createPhysicalSource called with body input', async () => {
    // createPhysicalSource returns void — sourceCode comes from the request input.
    createPhysicalSourceMock.mockResolvedValueOnce(undefined);
    const { ctrl, connectedAccounts } = buildController(makeIsvOptions());
    const res = makeMockResponse();
    await ctrl.createSource(
      'acc_test_001',
      { kind: 'physical', name: 'Counter 1', sourceCode: '5678' },
      res,
    );
    expect(res._status).toBe(201);
    const body = JSON.parse(res._body);
    ...
    expect(createPhysicalSourceMock.mock.calls[0]![0].sourceCode).toBe('5678');
    // Physical (POS) source codes must NOT become the web checkout source.
    expect(connectedAccounts.writeSourceCode).not.toHaveBeenCalled();
    expect(connectedAccounts.assignVivaPaymentMethod).not.toHaveBeenCalled();
  });

  it('4b. ecommerce: no channel for accountId → still 201, no persistence throw', async () => {
    createEcommerceSourceMock.mockResolvedValueOnce(undefined);
    const { ctrl, connectedAccounts } = buildController(makeIsvOptions());
    connectedAccounts.findChannelByAccountId.mockResolvedValueOnce(null);
    const res = makeMockResponse();
    await ctrl.createSource(
      'acc_test_001',
      { kind: 'ecommerce', domain: 'shop.example.com', pathSuccess: '/ok', pathFail: '/no', name: 'Web', sourceCode: '1234' },
      res,
    );
    expect(res._status).toBe(201);
    expect(connectedAccounts.writeSourceCode).not.toHaveBeenCalled();
  ...
  it('4c. ecommerce: persistence failure does not fail the request (201)', async () => {
    createEcommerceSourceMock.mockResolvedValueOnce(undefined);
    const { ctrl, connectedAccounts } = buildController(makeIsvOptions());
    connectedAccounts.writeSourceCode.mockRejectedValueOnce(new Error('db down'));
    const res = makeMockResponse();
    await ctrl.createSource(
      'acc_test_001',
      { kind: 'ecommerce', domain: 'shop.example.com', pathSuccess: '/ok', pathFail: '/no', name: 'Web', sourceCode: '1234' },
    ...
    expect(res._status).toBe(201);
    expect(JSON.parse(res._body).sourceCode).toBe('1234');
  });
