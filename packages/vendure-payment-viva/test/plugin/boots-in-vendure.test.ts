...
describe.skipIf(!pgReachable)('VivaPaymentPlugin boots inside a real Vendure instance', () => {
  ...
  // -------------------------------------------------------------------------
  // #28 — orderByVivaReturn Shop API Query is wired into the schema and
  // resolves through the real GraphQL layer. An unknown vivaOrderCode returns
  // null (the not-found member), proving the `extend type Query` extension is
  // registered and the resolver serializes a nullable Order without throwing.
  // The lookup/authz branches are unit-tested in
  // shop-api-order-by-viva-return.test.ts and exercised against the real DB by
  // the flow test below.
  // -------------------------------------------------------------------------
  it('#28 (runtime) — orderByVivaReturn query is in the schema and returns null for an unknown code', async () => {
    const data = await shopClient.query(
      parse(`
        query {
          orderByVivaReturn(vivaOrderCode: "does-not-exist-999") {
            id
            code
          }
        }
    ...
    expect(data.orderByVivaReturn).toBeNull();
  });
  ...
  // -------------------------------------------------------------------------
  // #28 flow — orderByVivaReturn resolves a Smart Checkout return to its Order
  // through the real DB: seed a real order + Payment + viva_transaction row, then
  // look it up by vivaOrderCode (the unique index, NOT the payment id). Auth is
  // satisfied via the anonymous active-order path (buildBootedResolver's Proxy
  // returns our order from getActiveOrderForUser); a non-owner gets null.
  // -------------------------------------------------------------------------
  it('#28 flow — owner resolves their order by vivaOrderCode; a non-owner gets null', async () => {
    const orderService = server.app.get(OrderService);
    const connection = server.app.get(TransactionalConnection);
    const paymentService = server.app.get(PaymentService);
    const realSM = server.app.get(StateMachineService);
    ...
    const total = 728;
    const orderCode = String(6652359299370000 + total);
    ...
    // Owner: the Proxy returns this order as the session's active order, so
    // _isOrderOwner passes. Real getVivaTransactionByOrderCode + findOne hit the DB.
    const ownerResolver = buildBootedResolver(order, realSM);
    const owned = (await ownerResolver.orderByVivaReturn(orderCode, ctx)) as any;
    expect(owned).not.toBeNull();
    expect(String(owned.id)).toBe(String(order.id));

    // Non-owner: getActiveOrderForUser returns a different order → null (the code
    // is indistinguishable from unknown, so it can't be enumerated).
    const strangerOrder = await orderService.create(ctx);
    const strangerResolver = buildBootedResolver(strangerOrder, realSM);
    const notOwned = await strangerResolver.orderByVivaReturn(orderCode, ctx);
    expect(notOwned).toBeNull();

    // Unknown code → null.
    const unknown = await ownerResolver.orderByVivaReturn('not-a-real-code', ctx);
