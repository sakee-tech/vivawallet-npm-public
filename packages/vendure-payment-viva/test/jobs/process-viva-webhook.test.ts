...
function buildProcessor(mocks: HandlerMocks) {
  ...
  return {
    ...
    async _handle1798(ctx: any, event: VivaWebhookEvent, mocks: HandlerMocks): Promise<void> {
      ...
      const orderCodeStr = eventData['OrderCode']?.toString();
...
describe('webhook 4865 — Order Updated (cancel detection)', () => {
  ...
  it('status X → cancels payment + transitions order to AddingItems', async () => {
    const event = makeEvent({
      eventTypeId: 4865,
      payload: {
        EventData: {
  ...
  it('non-cancel status → no cancel action, just marks processed', async () => {
    const event = makeEvent({
      eventTypeId: 4865,
      payload: { EventData: { OrderCode: 9876543210, StatusId: 'F', MerchantId: 'merchant-uuid' } },
    });
    mocks.eventRepo.findOne.mockResolvedValue(event);
    mocks.txnRepo.findOne.mockResolvedValue(makeVivaRow());
