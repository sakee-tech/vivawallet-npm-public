...
`cancelPayment` mutation + `orderByVivaReturn` query, and a
`vendure-viva-register-webhooks` CLI.
...
| **Storefront `orderByVivaReturn` query** | mounted | mounted |
...
On a **successful** return, Viva's ISV redirect carries only `?s=<vivaOrderCode>`
and `?t=<transactionId>` — neither is the Vendure order code. Resolve the order
first-party (no cross-site cookie) with `orderByVivaReturn`, then render the
confirmation:

```graphql
# Storefront: success return → resolve the Vendure order from ?s=<vivaOrderCode>
query OrderByVivaReturn($vivaOrderCode: String!) {
  orderByVivaReturn(vivaOrderCode: $vivaOrderCode) {
    id
    code
    state
  }
...
It is owner-scoped (active customer or active anonymous order, same as
`cancelPayment`) and returns `null` when the code is unknown or not owned by the
caller, so `?s=` can't be enumerated to leak orders.

On a **cancel/decline** return, handle the cancel case:
...
extend type Query {
  """
  Resolve the Vendure order for a Smart Checkout return (?s=<vivaOrderCode>).
  Indexed lookup on the plugin's own viva_transaction table (unique
  vivaOrderCode) → payment → order — no ISV API call. Owner-scoped to the
  active customer / active anonymous order, exactly like cancelPayment, so the
  code can't be enumerated to leak orders. Returns null when not found or not
  owned by the caller.
  """
  orderByVivaReturn(vivaOrderCode: String!): Order
}

extend type Mutation {
  """
  Cancel a Vendure payment by ID.
  Voids the Viva-side authorization (DELETE /api/orders/{orderCode} on the
  legacy host, Basic auth) AND transitions the order back to AddingItems.
