...
import type { PaymentProcess, Injector, PaymentTransitionData } from '@vendure/core';
import { StateMachineService } from './services/state-machine.service.js';
import { VIVA_LOG_CONTEXT } from './constants.js';

let stateMachine: StateMachineService | undefined;
...
export const vivaPaymentProcess: PaymentProcess<never> = {
  ...
  init(injector: Injector): void {
    stateMachine = injector.get(StateMachineService);
  },

  async onTransitionStart(_fromState, toState, data: PaymentTransitionData): Promise<void> {
    ...
    if (toState !== 'Created' || !stateMachine) return;
    const vivaOrderCode = (data.payment?.metadata as Record<string, unknown> | undefined)?.[
      'vivaOrderCode'
    ] as string | undefined;
    ...
    try {
      ...
      Logger.error(
        `[vivaPaymentProcess] Failed to reconcile viva_transaction.paymentId for ` +
          `vivaOrderCode=${vivaOrderCode}: ${err instanceof Error ? err.message : String(err)}`,
        VIVA_LOG_CONTEXT,
      );
    }
  },
