 ...
 *     registration per ISV-registerable event type (1796, 1797, 1798, 8193, 8194).
...
import type { DesiredWebhook, WebhookPlanAction, PlanResult, RunOptions } from './types.js';
import { computePlan, buildOwnedPattern } from './plan.js';
...
// onboarding events (8193, 8194) are ISV-only.
//
...
function detectMode(env: NodeJS.ProcessEnv): 'isv' | 'merchant' {
  const explicit = env['VIVA_MODE'];
  if (explicit === 'isv' || explicit === 'merchant') return explicit;
...
async function runMerchant(input: RunMerchantInput): Promise<number> {
  ...
  const environment = (env['VIVA_ENVIRONMENT'] ?? 'demo') as 'demo' | 'production';

  if (!merchantId || !apiKey) {
    console.error(
      '[vendure-viva-register-webhooks] Merchant-mode --apply requires VIVA_MERCHANT_ID and VIVA_API_KEY.',
    );
    return 3;
  }
  ...
  try {
    ...
    console.error(
      `[vendure-viva-register-webhooks] Failed to fetch verification key: ${msg}`,
    );
    return 2;
  }
...
async function runIsv(input: RunIsvInput): Promise<number> {
  ...
  if (!clientId && env['VIVA_ISV_CLIENT_ID']) {
    ...
    console.warn(
      '[vendure-viva-register-webhooks] VIVA_ISV_CLIENT_ID is deprecated, rename to VIVA_CLIENT_ID. Removal in 0.3.0.',
    );
  }
  ...
  if (!clientSecret && env['VIVA_ISV_CLIENT_SECRET']) {
    ...
    console.warn(
      '[vendure-viva-register-webhooks] VIVA_ISV_CLIENT_SECRET is deprecated, rename to VIVA_CLIENT_SECRET. Removal in 0.3.0.',
    );
  }
  const environment = (env['VIVA_ENVIRONMENT'] ?? 'demo') as 'demo' | 'production';
  ...
  try {
    ...
    const msg = err instanceof VivaApiError ? err.message : String(err);
    console.error(
      `[vendure-viva-register-webhooks] Could not fetch the ISV webhook verification key ` +
  ...
  if (!configuredKey) {
    console.error(
      `[vendure-viva-register-webhooks] VIVA_WEBHOOK_VERIFICATION_KEY is not set.\n` +
  ...
  if (configuredKey !== vivaIssuedKey) {
    console.error(
      `[vendure-viva-register-webhooks] VIVA_WEBHOOK_VERIFICATION_KEY does not match the Viva-issued ISV key.\n` +
  ...
  if (opts.output !== 'json') {
    console.log(
      '[vendure-viva-register-webhooks] ISV webhook verification key reconciled with Viva (GET /isv/v1/webhooks/token).',
    );
  }
  ...
  if (opts.reconcileDrift) {
    console.warn(
      '[vendure-viva-register-webhooks] --reconcile-drift is a no-op: the ISV API does not expose list/deactivate endpoints. ' +
  ...
  if (opts.reconcileDrift) {
    planInput.ownedHostnamePattern = buildOwnedPattern(webhookUrl);
  }
  ...
  if (opts.dryRun) {
    ...
    if (plan.hasFatalError) return 3;
  ...
  if (opts.apply) {
    if (plan.hasFatalError) {
      printPlan(plan, opts.output);
      console.error(
        '[vendure-viva-register-webhooks] Aborting: ABORT_LIMIT_HIT in plan. ' +
    ...
    for (const action of actionsToApply) {
      if (action.kind === 'REGISTER') {
        try {
          ...
          const msg = err instanceof VivaApiError ? err.message : String(err);
          // 409 Conflict → already registered, treat as no-change (not a failure)
          if (err instanceof VivaApiError && err.httpStatus === 409) {
    ...
    if (failures.length > 0) {
      console.error(
        `[vendure-viva-register-webhooks] ${applied} registered, ${failures.length} failed: ` +
...
function logLine(output: RunOptions['output'], level: string, message: string): void {
  if (output === 'json') {
