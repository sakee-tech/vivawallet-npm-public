...
async function runIsv(input: RunIsvInput): Promise<number> {
  ...
  // -------------------------------------------------------------------------
  const envKey = env['VIVA_WEBHOOK_VERIFICATION_KEY'];
  let generatedVerificationKey: string | undefined;

  if (!envKey) {
    generatedVerificationKey = randomUUID();
    console.warn(
      `[vendure-viva-register-webhooks] VIVA_WEBHOOK_VERIFICATION_KEY not set.\n` +
      `  Generated verification key: ${generatedVerificationKey}\n` +
      `  Set VIVA_WEBHOOK_VERIFICATION_KEY=${generatedVerificationKey} in your plugin options and .env.\n` +
      `  This key is returned by GET /viva/webhook to prove URL ownership to Viva.\n` +
      `  It is NOT posted to Viva — Viva fetches it from your endpoint during registration.`,
    );
  }
...
function printPlan(plan: PlanResult, output: RunOptions['output']): void {
      ...
      JSON.stringify(
        {
          ...
          generatedVerificationKey: plan.generatedVerificationKey,
        },
        null,
        2,
      ),
  ...
  if (plan.generatedVerificationKey) {
    console.log(`  ⚠  Generated VIVA_WEBHOOK_VERIFICATION_KEY: ${plan.generatedVerificationKey}`);
    console.log('     Set this value in your env and plugin config before running --apply.');
    console.log('');
  }
