 ...
 */
const CANCELLABLE_PAYMENT_STATES = new Set<string>(['Created', 'Authorized']);
 ...
 */
export function classifyCancel(paymentState: string): CancelDisposition {
  if (CANCELLABLE_PAYMENT_STATES.has(paymentState)) return 'proceed';
  if (paymentState === 'Settled') return 'refuse-paid';
