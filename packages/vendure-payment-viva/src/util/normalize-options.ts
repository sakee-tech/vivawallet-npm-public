...
export function _resetInitNoticesForTesting(): void {
...
const ISV_ONLY_KEYS: ReadonlyArray<keyof VivaPaymentPluginInitInput> = [
  'resolveMerchantId',
  'resolveSourceCode',
  'resolveIsvAmount',
...
const MERCHANT_ONLY_KEYS: ReadonlyArray<keyof VivaPaymentPluginInitInput> = [
  'sourceCode',
  'checkoutColor',
];

export function normalizePluginOptions(
  input: VivaPaymentPluginInitInput,
): VivaPaymentPluginOptions {
  const errors: string[] = [];
  ...
  if (!clientId && input.isvClientId) {
    ...
    if (!_isvClientIdAliasWarned) {
      ...
      console.warn(
        '[viva] `isvClientId` is deprecated, rename to `clientId`. ' +
          'Removal in 0.3.0.',
      );
    }
  }
  ...
  if (!clientSecret && input.isvClientSecret) {
    ...
    if (!_isvClientSecretAliasWarned) {
      ...
      console.warn(
        '[viva] `isvClientSecret` is deprecated, rename to `clientSecret`. ' +
          'Removal in 0.3.0.',
      );
    }
  }
  ...
  if (input.mode === 'merchant' || input.mode === 'isv') {
    ...
    if (!isvFieldsPresent && !_defaultModeNoticeShown) {
      ...
      console.warn(
        `[viva] mode is unset — defaulting to 'merchant'. ` +
  ...
  if (mode === 'merchant' && isvFieldsPresent && !_merchantWithIsvResolversWarned) {
    ...
    console.warn(
      `[viva] mode='merchant' but ISV-only field(s) supplied: ${presentKeys.join(', ')}. ` +
  ...
  if (mode === 'merchant') {
    if (!input.legacyMerchantId) {
      errors.push(
        'legacyMerchantId is required in merchant mode — Merchant Basic auth for createRefund.',
      );
    }
    if (!input.legacyApiKey) {
      errors.push(
        'legacyApiKey is required in merchant mode — Merchant Basic auth for createRefund.',
      );
    }
  }
  ...
  if (mode === 'isv' && input.onboardingReturnUrl === undefined) {
    errors.push(
      "onboardingReturnUrl is required when mode='isv' " +
  ...
  if (mode === 'merchant') {
    const merchant: VivaMerchantOptions = {
      ...common,
      mode: 'merchant',
