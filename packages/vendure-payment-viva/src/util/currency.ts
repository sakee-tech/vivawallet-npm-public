 ...
 */
const ALPHA_TO_NUMERIC: Readonly<Record<string, string>> = {
  EUR: '978',
  GBP: '826',
  USD: '840',
  PLN: '985',
...
const NUMERIC_TO_ALPHA: Readonly<Record<string, string>> = Object.fromEntries(
  Object.entries(ALPHA_TO_NUMERIC).map(([alpha, numeric]) => [numeric, alpha]),
);
 ...
 */
export function alphaToNumeric(alpha: string): CurrencyCode {
 ...
 * @example numericToAlpha(826) // → 'GBP'
 */
export function numericToAlpha(numeric: string | number): string {
 ...
 */
export function coerceCurrencyCode(raw: string | number): CurrencyCode {
