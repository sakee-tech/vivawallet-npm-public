...
import { shouldRefuseCancel } from '../util/cancel-guard.js';
...
export class VivaShopApiResolver {
  ...
  async cancelPayment(
    ...
    // Only enforceable when a row exists; the metadata-fallback path has no row
    // status to inspect — the handler's Viva DELETE is idempotent for an
    // already-terminal order, so deferring the check there is safe.
    //
    // CRITICAL (#26): this guard MUST share the handler's carve-out via
    // shouldRefuseCancel(). A `captured` row whose Payment never reached
    // `Settled` is the reconcilable desync — refusing it here shadows the
    // handler's reconcile in Step 7 (the only path that can clear it), so the
    // Created payment never frees and every retry bricks on
    // isvAmountTooHigh(_, 0). For that one case we fall through to Step 7;
    // every other terminal status is still refused.
    // -------------------------------------------------------------------------
    if (shouldRefuseCancel(vivaRow?.status, payment.state)) {
      return cancelPaymentError({
        errorCode: 'VIVA_PAYMENT_NOT_CANCELLABLE',
