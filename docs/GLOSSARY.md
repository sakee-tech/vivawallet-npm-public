...
|---|---|---|---|---|
| **`orderCode`** | Viva (returned from `POST /checkout/v2/orders`) | `viva_transaction.order_code` | Constructing the Smart Checkout redirect URL (`?ref={orderCode}`); cancel order URL (`DELETE /api/orders/{orderCode}` on the legacy host) | 16-digit numeric — `1234567890123456` |
