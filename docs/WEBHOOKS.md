...
| `ResponseEventId` | int \| null | Viva-internal transaction EventId (internal-operations logging) — **not** a public decline-reason map; the decline reason is in `ResponseCode` |
...
> **ISV registration caveat:** `4865` is **not registerable via the ISV webhook API** (`POST /isv/v1/webhooks` rejects it with `IsvCreateWebhookFailedInvalidEventTypeId`). The `register-webhooks` CLI therefore omits it from the ISV set (`ISV_EVENT_TYPE_IDS` = `1796, 1797, 1798, 8193, 8194`). Under ISV, Order Updated can only be registered **per connected merchant** via that merchant's Self-Care. The webhook *handler* still accepts `4865` inbound in all modes (it remains in `V1_EVENT_TYPE_IDS`). See sakee-tech/vivawallet-npm-public#18.
