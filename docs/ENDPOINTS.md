...
```
DELETE https://{oauth-api}/checkout/v2/orders/{orderCode}
DELETE https://{oauth-api}/checkout/v2/orders/{orderCode}?merchantId={uuid}    # ISV
```

| | |
|---|---|
| **Auth** | OAuth2 — `redirectcheckout` (M) / `isv redirectcheckout` (I) |
| **Modes** | M, I |
| **Effect** | Voids an unpaid Smart Checkout order (storefront `?paymentCancelled=1` flow). Idempotent — safe to retry. |
...
### 4.2 Standard refund (Basic, universal)

```
POST https://{basic-api}/api/transactions/{transactionId}
```
...
| **Modes** | M, I |
| **Body** | Form-urlencoded (`Content-Type: application/x-www-form-urlencoded`): `Amount=<minor-units>&SourceCode=<string>` |
| **Response 200** | `{ "TransactionId": "<uuid>", "EventId": <number>, "Amount": <number> }` (note: PascalCase keys) |
...
| **Webhook trigger** | 1797 (Transaction Reversal Created) — `EventData.ServiceId` ≠ 19 |
| **Probe note** | F1 (2026-04-25): the OAuth2 v2 path `POST /checkout/v2/transactions/{id}` returns **405** — that path does NOT support refund. This Basic endpoint is the only universal refund path. |
| **Spec ref** | `payment-api.yaml:4993`, `payment-isv-api.yaml:3611` |
