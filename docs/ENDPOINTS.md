...
### 3.2b Retrieve order (by orderCode)

```
GET https://{legacy-host}/api/orders/{orderCode}
```
...
| **Host** | **Legacy host** (`demo.vivapayments.com` / `www.vivapayments.com`) — there is **no** v2/OAuth2 equivalent (that route 404s, same as cancel §3.3) |
| **Modes** | M, I |
| **Response fields** | `OrderCode` (int64 — quote before JSON-parse to preserve precision), `StateId`, `RequestAmount` / `MaxInstallments` — amounts are **major-unit floats with no currency in the body**; the helper applies `opts.currencyCode` for non-2-decimal currencies. |
| **Plugin usage** | `Payments.retrieveOrder(orderCode, opts?)`. Idempotent. Requires a `legacyClient`. |
| **Note** | Distinct from §3.1 retrieve-*transaction* (OAuth2, by transactionId). Use this to inspect an order's lifecycle state by its `orderCode`. |
