...
| Cancel order (void auth) | `DELETE /checkout/v2/orders/{orderCode}` | OAuth2 | `urn:viva:payments:core:api:redirectcheckout` (`...:isv` for ISV) | `*-api.vivapayments.com` |
...
| **Standard refund** | `POST /api/transactions/{transactionId}` | Basic | Merchant Basic (`MerchantId:ApiKey`) | `*.vivapayments.com` | All cards / all methods / all merchants; T+2 settlement window. Probe F1 (2026-04-25): OAuth2 v2 path returns 405; this is the only universally-working refund path. |
...
|---|---|
| `POST /api/orders` (Basic) | "Use `/checkout/v2/orders` instead" — current code already does |
...
The endpoints the plugin uses today (`POST /api/transactions/{id}` for refund, `POST /api/sources` for source creation, `GET /api/messages/config/token` for webhook key) are **not** marked deprecated.
...
|---|---|---|---|
| `merchant` | merchant's own pair → `redirectcheckout` (+ `acquiring`/`acquiring:transactions` if Fast Refund enabled) | one `BasicAuthClient` in `'merchant'` variant | OAuth2 for checkout/retrieve/cancel/Fast-Refund; Basic for standard refund + webhook key fetch |
| `isv` | platform pair → `isv redirectcheckout` (+ acquiring scopes for Fast Refund) | two `BasicAuthClient` instances: `'merchant'` variant for refund (using platform's own merchant creds) and `'reseller'` variant for source creation (using reseller creds) | OAuth2 for checkout/retrieve/cancel/Fast-Refund/onboarding/webhook-reg; Basic-merchant for refund; Basic-reseller for source creation |
| `marketplace` *(reserved)* | merchant's own pair + `platform` scope | `'merchant'` variant Basic | TBD |
...
| `legacyMerchantId` / `legacyApiKey` | `BasicAuthClient` (merchant variant) | always — webhook key + standard refund |
| `reseller.resellerId` / `reseller.merchantId` / `reseller.resellerApiKey` | `BasicAuthClient` (reseller variant) | only when `mode='isv'` and plugin will create payment sources |
...
Calling Viva to … ?
  ...
  Refund — otherwise
      → Basic (Merchant)  →  POST /api/transactions/{id} on *.vivapayments.com
...
| Cancel order | `DELETE https://demo-api.vivapayments.com/checkout/v2/orders/{orderCode}` | `DELETE https://api.vivapayments.com/checkout/v2/orders/{orderCode}` |
...
| Standard refund | `POST https://demo.vivapayments.com/api/transactions/{id}` | `POST https://www.vivapayments.com/api/transactions/{id}` |
