...
| `cancelPayment()` mutation (Vendure) / equivalent (Medusa) | Plugin calls `DELETE /api/orders/{orderCode}` (legacy host, Basic auth) AND transitions order to `AddingItems` | `initiated → cancelled` (synchronously) |
